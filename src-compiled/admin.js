(function() {
  ...
  const bookingActivity = (b) => {
    ...
    if (dep && a <= T && T <= dep) {
      ...
      return { when: "today", label: act ? "Today · " + act : "On trip today" };
    }
    if (dep && a <= TM && TM <= dep) {
      const act = itinOn(b, TM);
      return { when: "tomorrow", label: act ? "Tomorrow · " + act : "On trip tomorrow" };
    }
    return null;
  };
  ...
  function Login({ onAuthed }) {
    ...
    const [email, setEmail] = useState(LEGACY_ADMIN_EMAIL);
  ...
  function MonthCalendar({ bookings, sel, onSelect, compact, year }) {
    ...
    if (year) {
      const miniMonth = (mi) => {
        const first2 = new Date(Y, mi, 1).getDay();
        const dim2 = new Date(Y, mi + 1, 0).getDate();
        const cells2 = [];
        ["S", "M", "T", "W", "T", "F", "S"].forEach((d, i) => cells2.push(h("span", { key: "h" + i, className: "msa-yr-dow" }, d)));
  ...
  function TeamActivity() {
    ...
    return h(
      ...
      partner ? h(
        ...
        h("span", { className: "msa-dim" }, online ? "Active now · " + (partner.current_view || "console") : "Last seen " + (agoOf(partner.last_seen) || "?") + " ago")
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              h("span", { className: "msa-collab-box-t" }, ICON.van ? ICON.van() : null, "🚗 Driver / transport"),
          ...
          h(
            ...
            h(
              "div",
              { className: "msa-collab-box-head" },
              h("span", { className: "msa-collab-box-t" }, "🏨 Accommodation"),
        ...
        initial && initial.id && initial.email ? h(
          ...
          h(
            ...
            thread.length === 0 ? h("div", { className: "msa-dim", style: { padding: "8px 0" } }, "No messages yet.") : thread.map((m) => h(
              ...
              h("div", { className: "msa-msg-meta" }, (m.sender === "admin" ? "You" : b.client_name || "Client") + " · " + fmtDateTime(m.created_at))
            ))
          ),
          h(
            "div",
  ...
  async function buildBookingPdf(b, S, opts) {
    ...
    if (b.address || b.email || b.phone) {
      ...
      const cp = [b.email, b.phone].filter(Boolean).join("   ·   ");
      if (cp) {
        T(cp, MX, y);
        y += 12;
      }
  ...
  function Clients({ clients, bookings, reload, initialQuery }) {
    ...
    const add = async () => {
      ...
      await dbInsert("clients", { ...f, email: f.email ? f.email.toLowerCase() : null, trips: 0, created_by: CURRENT_EMAIL || LEGACY_ADMIN_EMAIL });
          ...
          return h(
            ...
            itin.length > 0 && expanded["itin-" + b.id] ? h("div", { className: "msa-cl-itin" }, itin.map((d, i) => h(
              ...
              h(
                ...
                (d.activities || []).map((a, ai) => h("div", { key: ai, className: "msa-cl-itin-act" }, (a.time ? a.time + " · " : "") + (a.type || "") + (a.details ? " — " + a.details : "")))
              )
            ))) : null,
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Clients"), h("p", { className: "msa-subtitle" }, clients.length + " clients · " + kr(clients.reduce((s, c) => s + (+c.total_spent || 0), 0)) + " lifetime value")),
  ...
  function Suppliers({ suppliers, bookings, leads, reload, seed, clearSeed }) {
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Collaborators"), h("p", { className: "msa-subtitle" }, suppliers.length + " partners · " + pending.length + " pending requests")),
  ...
  function Workspace({ tasks, reload }) {
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Workspace"), h("p", { className: "msa-subtitle" }, "Shared to-dos for the team · " + open.length + " open")),
      ...
      return h("div", { className: "msa-cal-progs" }, list.map(({ b, kinds }) => {
        ...
        return h(
          ...
          h(
            ...
            acts.length ? acts.map((a, i) => h("div", { key: i, className: "msa-cal-act" }, h("span", { className: "msa-cal-act-t" }, a.time || "·"), h("div", null, h("strong", null, a.type || ""), a.details ? h("p", null, a.details) : null))) : prog ? null : h("div", { className: "msa-dim", style: { fontSize: 12.5, padding: "4px 2px" } }, "No detailed program set for this day yet."),
    ...
    const row = (l) => {
      ...
      const main = h(
        ...
        h(
          "td",
          { "data-label": "Booked" },
          title === "—" ? h("span", { className: "msa-dim" }, [l.trip_type, l.duration ? l.duration + "d" : null].filter(Boolean).join(" · ") || "—") : h("strong", { className: "msa-text-brand" }, title),
      ...
      const detail = h("tr", { key: l.id + "-d", className: "msa-bt-detail" }, h(
        ...
        itin.length > 0 ? h("div", { className: "msa-rq-itin", style: { marginTop: 12 } }, itin.map((d, i) => h(
          ...
          h(
            ...
            null,
            h("strong", null, (d.city || "Day " + (d.day || i + 1)) + (d.date ? " · " + fmtDate(d.date) : "")),
            (d.activities || []).map((a, ai) => h("div", { key: ai, className: "msa-dim" }, (a.time ? a.time + " · " : "") + (a.type || "") + (a.details ? " — " + a.details : "")))
          )
        ))) : null,
      ...
      h(
        ...
        h(
          ...
          h(
            "div",
            null,
            h("div", { className: "sm-inv-ch" }, "COORDONNÉES :"),
    ...
    const gaTrend = (data, label) => {
      ...
      return h(
        ...
        !d.length ? h("div", { className: "msa-empty" }, "No data yet") : h("div", { className: "msa-ga-trend" }, d.map((x, i) => h(
          "div",
          { key: i, className: "msa-ga-trend-col", title: nf(x.value) + " users · " + gaDate(x.key) },
