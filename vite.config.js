  ...
  return {
    ...
    closeBundle() {
      ...
      const extra = [
        ...
        `  EXPECTED_SUPABASE_PROJECT_REF: ${JSON.stringify(env.EXPECTED_SUPABASE_PROJECT_REF || '')}`,
      ];
...
export default defineConfig(({ mode }) => {
  ...
  const configuredUrl = env.VITE_SUPABASE_URL || env.NEXT_PUBLIC_SUPABASE_URL || '';
  const expectedRef = env.EXPECTED_SUPABASE_PROJECT_REF || '';
  const actualRef = (() => { try { return new URL(configuredUrl).hostname.split('.')[0]; } catch (_) { return ''; } })();
  if (mode === 'production' && expectedRef && actualRef !== expectedRef) {
    throw new Error(`[supabase-guard] Production Supabase project mismatch. Expected ${expectedRef}, got ${actualRef || '(invalid URL)'}.`);
  }
  return {
    appType: 'mpa',
    build: {
