...
function generateEnvJs(env) {
  return {
    ...
    closeBundle() {
      ...
      const siteUrl = env.VITE_SITE_URL || env.NEXT_PUBLIC_SITE_URL || '';
      const adminUrl = env.VITE_ADMIN_URL || env.NEXT_PUBLIC_ADMIN_URL || '';
      if (!url || !key) {
      ...
      const extra = [];
      if (siteUrl) extra.push(`  SITE_URL: ${JSON.stringify(siteUrl)}`);
      if (adminUrl) extra.push(`  ADMIN_URL: ${JSON.stringify(adminUrl)}`);
