(function() {
  ...
  const PREFER_REDUCED = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  ...
  const bindTilt = (root = document) => {
    ...
    els.forEach((el) => {
      ...
      const onMove = (e) => {
        ...
        const tx = (x - 0.5) * 2 * maxDeg;
        const ty = (0.5 - y) * 2 * maxDeg;
  ...
  const autoTag = () => {
    document.querySelectorAll("section.section, .itin-section, .reiseplaner-section, #catalog, #contact, #agafay").forEach((s) => s.classList.add("fx-reveal"));
    document.querySelectorAll(".cat-grid, .ag-grid, .reiseplaner-grid").forEach((g) => g.classList.add("fx-reveal-stagger"));
    document.querySelectorAll(".cat-card, .ag-card, .opt-card, .dur-pkg-card, .rental-card, .ip-phone-stage").forEach((c) => {
      if (!c.classList.contains("fx-tilt")) c.classList.add("fx-tilt");
    });
    document.querySelectorAll(".ms-hero-bg").forEach((b) => {
      b.classList.add("fx-parallax");
      b.dataset.speed = "0.15";
    });
  };
  autoTag();
