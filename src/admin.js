(function() {
  ...
  const isAdminRole = () => CURRENT_ROLE === "admin";
  ...
  function getSB() {
    ...
    try {
      ...
      window.MS_DB_CONFIG_ERROR = "Database configuration error: invalid Supabase URL. Data operations are blocked.";
      return null;
    }
  ...
  async function resetAdminAuthState() {
    ...
    try {
      if (window.MS_SB && window.MS_SB.auth && window.MS_SB.auth.signOut) await window.MS_SB.auth.signOut();
    } catch (_e) {
    }
  }
  async function dbList(t, order, asc) {
    const sb = getSB();
    if (!sb) return [];
    ...
    if (error) {
      console.warn("[admin]", t, error.message);
      return [];
    }
    return data || [];
  }
  async function dbListAll(t, order, asc) {
    const sb = getSB();
    if (!sb) return [];
    const all = [];
    for (let offset = 0; offset < 1e5; offset += 1e3) {
      let q = sb.from(t).select("*").range(offset, offset + 999);
      if (order) q = q.order(order, { ascending: asc !== false });
      const { data, error } = await q;
      if (error) {
        console.warn("[admin]", t, error.message);
  ...
  async function dbInsert(t, row) {
    ...
    return await sb.from(t).insert(row).select();
  }
  async function dbUpdate(t, id, patch) {
    const sb = getSB();
    if (!sb) return { error: "no client" };
    return await sb.from(t).update(patch).eq("id", id).select();
  }
  async function dbDelete(t, id) {
    const sb = getSB();
    if (!sb) return { error: "no client" };
    return await sb.from(t).delete().eq("id", id);
  }
  async function logAudit(action, entity, entityId, detail) {
    try {
      const sb = getSB();
      if (!sb || !CURRENT_EMAIL) return;
  ...
  async function touchPresence(view) {
    try {
      const sb = getSB();
      if (!sb || !CURRENT_EMAIL) return;
      await sb.from("staff_presence").upsert({ email: CURRENT_EMAIL, name: CURRENT_NAME, role: CURRENT_ROLE, current_view: view || null, last_seen: (/* @__PURE__ */ new Date()).toISOString(), updated_at: (/* @__PURE__ */ new Date()).toISOString() }, { onConflict: "email" });
    } catch (e) {
    }
  }
  ...
  const isAdminRecoveryLanding = () => {
    ...
    return search.get("admin_recovery") === "1" || search.get("type") === "recovery" || /(^|[&#])type=recovery(?:&|$)/.test(hash);
  };
  ...
  function reqKindLabel(l) {
    return l && REQ_KIND[l.kind] || l && l.kind || "Request";
  }
  function fmtReqVal(v) {
    ...
    if (Array.isArray(v)) {
      const a = v.map(reqText).filter(Boolean);
  ...
  const REQ_FIELD = {
    ...
    email: { l: "Email", g: "who" },
    phone: { l: "Phone", g: "who" },
    country: { l: "Country", g: "who" },
    people: { l: "Travellers", g: "who" },
    vanSeats: { l: "Van seats", g: "who" },
    tripType: { l: "Trip type", g: "trip" },
    duration: { l: "Duration (days)", g: "trip" },
    startDate: { l: "Start date", g: "trip" },
    multiCity: { l: "Multi-city", g: "trip" },
    arriveCity: { l: "Arrival city", g: "trip" },
    departCity: { l: "Departure city", g: "trip" },
    pace: { l: "Pace", g: "style" },
    budget: { l: "Budget", g: "style" },
    accommodation: { l: "Accommodation", g: "style" },
    flex: { l: "Flexibility", g: "style" },
    interests: { l: "Interests", g: "style" },
    stops: { l: "Stops", g: "style" },
    occasion: { l: "Occasion", g: "style" },
    daySchedule: { l: "Day schedule", g: "style" },
    flightBooked: { l: "Flights booked", g: "log" },
    bookedAccom: { l: "Accommodation booked", g: "log" },
    bookedTransport: { l: "Transport booked", g: "log" },
    bookedActivities: { l: "Activities booked", g: "log" },
    notes: { l: "Notes", g: "notes" },
    avoid: { l: "Avoid", g: "notes" }
  };
  ...
  const reqOrderIdx = (k) => {
    if (k === "notes") return 90001;
    if (k === "avoid") return 90002;
    const i = REQ_ORDER.indexOf(k);
    return i < 0 ? 5e4 : i;
  };
  function bkDayProgram(b, k) {
    ...
    if (!m && b.arrival_date) {
      const diff = Math.round((new Date(k) - new Date(b.arrival_date.slice(0, 10))) / 864e5);
      if (diff >= 0 && diff < itin.length) m = itin[diff];
    }
    return m;
  }
  function bkDayActs(b, k) {
    if (k === (b.arrival_date || "").slice(0, 10)) return "Arrival \xB7 " + (b.arrival_city || "Marrakech");
    if (k === (b.departure_date || "").slice(0, 10)) return "Departure \xB7 " + (b.departure_city || "");
    const prog = bkDayProgram(b, k);
    if (!prog) return "";
    const acts = (Array.isArray(prog.activities) ? prog.activities : []).map((a) => a.type || a.details).filter(Boolean).slice(0, 3).join(" \xB7 ");
  ...
  function agentDraft(l) {
    ...
    const factLine = facts.join(" \xB7 ");
    if (lang === "no" || lang === "nb") {
      return [
        "Hei " + first + ",",
        "",
        "Tusen takk for foresp\xF8rselen din til MarrakechStory \u2014 s\xE5 hyggelig at du vil reise med oss!",
        factLine ? "Vi har notert oss: " + factLine + "." : "",
        chooseForMe ? "Siden du \xF8nsker at vi skreddersyr turen, setter vi sammen et komplett dag-for-dag-program for deg." : "Vi setter sammen et forslag som passer \xF8nskene dine.",
        p.notes ? "Vi har lest notatene dine og tar med \xF8nskene i planen." : "",
        "",
        "Du f\xE5r et skreddersydd forslag med program og pris innen 24 timer. Si gjerne ifra om datoene er faste, s\xE5 l\xE5ser vi de beste riadene og opplevelsene med en gang.",
        "",
        "Varme hilsener,",
        "Aladdin & Marte \u2014 MarrakechStory"
      ].filter((x) => x !== "").join("\n");
    }
    return [
      "Hi " + first + ",",
  ...
  const daysUntil = (d) => {
    ...
    const a = new Date(d);
    a.setHours(0, 0, 0, 0);
    return Math.round((a - startOfToday()) / 864e5);
  };
  ...
  const itinOn = (b, day) => {
    const it = Array.isArray(b.daily_itinerary) ? b.daily_itinerary : [];
    const d = it.find((x) => dayOf(x.date) === day);
    if (!d) return "";
    const acts = (d.activities || []).map((a) => a.type || a.details).filter(Boolean);
    return d.city || acts[0] || "";
  };
  const bookingActivity = (b) => {
    if (!b || b.status === "cancelled") return null;
    const T = localDayStr(/* @__PURE__ */ new Date()), TM = shiftDay(T, 1);
    const a = dayOf(b.arrival_date), dep = dayOf(b.departure_date);
    if (!a) return null;
    const tail = (s) => s ? " \xB7 " + s : "";
    ...
    if (dep && a <= T && T <= dep) {
      const act = itinOn(b, T);
      return { when: "today", label: act ? "Today \xB7 " + act : "On trip today" };
    }
    if (dep && a <= TM && TM <= dep) {
      const act = itinOn(b, TM);
      return { when: "tomorrow", label: act ? "Tomorrow \xB7 " + act : "On trip tomorrow" };
    }
    return null;
  };
  ...
  const CONFIRMED_BOOKING_STATUSES = /* @__PURE__ */ new Set(["confirmed", "deposit_paid", "fully_paid", "ongoing", "completed"]);
  ...
  const CUR_SYMBOL = { NOK: "kr", SEK: "kr", DKK: "kr", MAD: "DH", EUR: "\u20AC", USD: "$", GBP: "\xA3" };
  const money = (n, cur) => {
    const v = Math.round(+n || 0).toLocaleString("nb-NO");
    const c = cur || "NOK";
    return c === "EUR" || c === "USD" || c === "GBP" ? CUR_SYMBOL[c] + " " + v : v + " " + (CUR_SYMBOL[c] || c);
  };
  ...
  const DOC_LANGS = [["no", "Norsk"], ["en", "English"], ["sv", "Svenska"], ["fr", "Fran\xE7ais"], ["de", "Deutsch"]];
  const DOC_LOCALE = { no: "nb-NO", en: "en-GB", sv: "sv-SE", fr: "fr-FR", de: "de-DE" };
  const DOC_TR = {
    itin_kicker: { no: "Privat reiseplan", en: "Private travel itinerary", sv: "Privat resplan", fr: "Itin\xE9raire priv\xE9", de: "Private Reiseroute" },
    itin_title: { no: "Reiseplan", en: "Itinerary", sv: "Resplan", fr: "Itin\xE9raire", de: "Reiseplan" },
    for: { no: "For", en: "For", sv: "F\xF6r", fr: "Pour", de: "F\xFCr" },
    trip: { no: "Reise", en: "Trip", sv: "Resa", fr: "Voyage", de: "Reise" },
    dates: { no: "Datoer", en: "Dates", sv: "Datum", fr: "Dates", de: "Daten" },
    travelers: { no: "Reisende", en: "Travellers", sv: "Resen\xE4rer", fr: "Voyageurs", de: "Reisende" },
    ref: { no: "Ref", en: "Ref", sv: "Ref", fr: "R\xE9f", de: "Ref" },
    persons: { no: "personer", en: "travellers", sv: "personer", fr: "voyageurs", de: "Personen" },
    nights: { no: "netter", en: "nights", sv: "n\xE4tter", fr: "nuits", de: "N\xE4chte" },
    day: { no: "Dag", en: "Day", sv: "Dag", fr: "Jour", de: "Tag" },
    no_itin: { no: "Ingen dagsplan lagt til enn\xE5.", en: "No daily itinerary added yet.", sv: "Inget dagsprogram tillagt \xE4nnu.", fr: "Aucun programme journalier ajout\xE9.", de: "Noch kein Tagesprogramm hinzugef\xFCgt." },
    tbd: { no: "Kommer", en: "TBD", sv: "Kommer", fr: "\xC0 d\xE9finir", de: "Offen" },
    included: { no: "Inkludert", en: "Included", sv: "Ing\xE5r", fr: "Inclus", de: "Inbegriffen" },
    not_included: { no: "Ikke inkludert", en: "Not included", sv: "Ing\xE5r inte", fr: "Non inclus", de: "Nicht inbegriffen" },
    foot_thanks: { no: "Takk for at du valgte MarrakechStory. Vi \xF8nsker deg en uforglemmelig reise.", en: "Thank you for choosing MarrakechStory. We wish you an unforgettable journey.", sv: "Tack f\xF6r att du valde MarrakechStory. Vi \xF6nskar dig en of\xF6rgl\xF6mlig resa.", fr: "Merci d'avoir choisi MarrakechStory. Nous vous souhaitons un voyage inoubliable.", de: "Danke, dass Sie MarrakechStory gew\xE4hlt haben. Wir w\xFCnschen Ihnen eine unvergessliche Reise." },
    ...
    billed_to: { no: "Fakturert til", en: "Billed to", sv: "Fakturerad till", fr: "Factur\xE9 \xE0", de: "Rechnung an" },
    invoice_no: { no: "Faktura", en: "Invoice", sv: "Faktura", fr: "Facture", de: "Rechnung" },
    date: { no: "Dato", en: "Date", sv: "Datum", fr: "Date", de: "Datum" },
    method: { no: "Metode", en: "Method", sv: "Metod", fr: "M\xE9thode", de: "Methode" },
    status: { no: "Status", en: "Status", sv: "Status", fr: "Statut", de: "Status" },
    bill_to: { no: "Fakturamottaker", en: "Bill to", sv: "Faktureras till", fr: "Facturer \xE0", de: "Rechnungsempf\xE4nger" },
    description: { no: "Beskrivelse", en: "Description", sv: "Beskrivning", fr: "Description", de: "Beschreibung" },
    amount: { no: "Bel\xF8p", en: "Amount", sv: "Belopp", fr: "Montant", de: "Betrag" },
    package: { no: "Skreddersydd reisepakke", en: "Bespoke Travel Package", sv: "Skr\xE4ddarsytt resepaket", fr: "Forfait voyage sur mesure", de: "Ma\xDFgeschneidertes Reisepaket" },
    travelers_word: { no: "reisende", en: "travellers", sv: "resen\xE4rer", fr: "voyageurs", de: "Reisende" },
    subtotal: { no: "Delsum", en: "Subtotal", sv: "Delsumma", fr: "Sous-total", de: "Zwischensumme" },
    deposit_confirm: { no: "Depositum for \xE5 bekrefte", en: "Deposit to confirm", sv: "Handpenning att bekr\xE4fta", fr: "Acompte pour confirmer", de: "Anzahlung zur Best\xE4tigung" },
    paid: { no: "Betalt", en: "Paid", sv: "Betalt", fr: "Pay\xE9", de: "Bezahlt" },
    balance_due: { no: "Restbel\xF8p", en: "Balance Due", sv: "\xC5terst\xE5ende", fr: "Solde d\xFB", de: "Restbetrag" },
    to_pay: { no: "\xC5 betale", en: "To pay", sv: "Att betala", fr: "\xC0 payer", de: "Zu zahlen" },
    total_amount: { no: "Totalbel\xF8p", en: "Total amount", sv: "Totalbelopp", fr: "Montant total", de: "Gesamtbetrag" },
    rest_to_pay: { no: "Resten \xE5 betale", en: "Rest to pay", sv: "Resten att betala", fr: "Reste \xE0 payer", de: "Restzahlung" },
    paynote_deposit: { no: "Et depositum p\xE5 {dep} ({pct}%) bekrefter bestillingen. Resten p\xE5 {rest} betales f\xF8r avreise.", en: "A deposit of {dep} ({pct}%) confirms your booking. The remaining {rest} is due before departure.", sv: "En handpenning p\xE5 {dep} ({pct}%) bekr\xE4ftar bokningen. Resterande {rest} betalas f\xF6re avresa.", fr: "Un acompte de {dep} ({pct}%) confirme votre r\xE9servation. Le solde de {rest} est d\xFB avant le d\xE9part.", de: "Eine Anzahlung von {dep} ({pct}%) best\xE4tigt Ihre Buchung. Der Restbetrag von {rest} ist vor Abreise f\xE4llig." },
    paynote_full: { no: "Full betaling p\xE5 {sub} kreves for \xE5 bekrefte bestillingen.", en: "Full payment of {sub} is required to confirm your booking.", sv: "Full betalning p\xE5 {sub} kr\xE4vs f\xF6r att bekr\xE4fta bokningen.", fr: "Le paiement int\xE9gral de {sub} est requis pour confirmer votre r\xE9servation.", de: "Die vollst\xE4ndige Zahlung von {sub} ist zur Best\xE4tigung Ihrer Buchung erforderlich." },
    bank_details: { no: "Bankoverf\xF8ring", en: "Bank transfer details", sv: "Bank\xF6verf\xF6ring", fr: "Coordonn\xE9es bancaires", de: "Bankverbindung" },
    ...
    cash_title: { no: "Kontant betaling", en: "Payment in cash", sv: "Kontant betalning", fr: "Paiement en esp\xE8ces", de: "Barzahlung" },
    label_method: { no: "Metode:", en: "Method:", sv: "Metod:", fr: "M\xE9thode :", de: "Methode:" },
    label_when: { no: "N\xE5r:", en: "When:", sv: "N\xE4r:", fr: "Quand :", de: "Wann:" },
    cash_when: { no: "Betales kontant ved ankomst i Marrakech", en: "Payable in cash on arrival in Marrakech", sv: "Betalas kontant vid ankomst i Marrakech", fr: "Payable en esp\xE8ces \xE0 l'arriv\xE9e \xE0 Marrakech", de: "Zahlbar in bar bei Ankunft in Marrakesch" },
    cash_word: { no: "Kontant", en: "Cash", sv: "Kontant", fr: "Esp\xE8ces", de: "Bar" },
    terms: { no: "Vilk\xE5r og betingelser", en: "Terms & Conditions", sv: "Villkor", fr: "Conditions g\xE9n\xE9rales", de: "Gesch\xE4ftsbedingungen" }
  };
  const ACT_TR = {
    "Transport": { no: "Transport", sv: "Transport", fr: "Transport", de: "Transport" },
    "Airport Transfer": { no: "Flyplasstransport", sv: "Flygplatstransfer", fr: "Transfert a\xE9roport", de: "Flughafentransfer" },
    "Private Driver": { no: "Privatsj\xE5f\xF8r", sv: "Privat chauff\xF6r", fr: "Chauffeur priv\xE9", de: "Privatfahrer" },
    "Check-in": { no: "Innsjekk", sv: "Incheckning", fr: "Arriv\xE9e", de: "Check-in" },
    "Check-out": { no: "Utsjekk", sv: "Utcheckning", fr: "D\xE9part", de: "Check-out" },
    "Guided Tour": { no: "Guidet tur", sv: "Guidad tur", fr: "Visite guid\xE9e", de: "Gef\xFChrte Tour" },
    "City Tour": { no: "Byrundtur", sv: "Stadsrundtur", fr: "Visite de la ville", de: "Stadtrundfahrt" },
    "Medina Tour": { no: "Medina-tur", sv: "Medina-tur", fr: "Visite de la m\xE9dina", de: "Medina-Tour" },
    "Ourika Valley": { no: "Ourika-dalen", sv: "Ourikadalen", fr: "Vall\xE9e de l'Ourika", de: "Ourika-Tal" },
    "Atlas Mountains": { no: "Atlasfjellene", sv: "Atlasbergen", fr: "Montagnes de l'Atlas", de: "Atlasgebirge" },
    "Agafay Day Pass": { no: "Agafay dagspass", sv: "Agafay dagspass", fr: "Pass journ\xE9e Agafay", de: "Agafay Tagespass" },
    "Agafay Dinner": { no: "Agafay middag", sv: "Agafay middag", fr: "D\xEEner \xE0 Agafay", de: "Agafay Abendessen" },
    "Desert Camp": { no: "\xD8rkenleir", sv: "\xD6kenl\xE4ger", fr: "Camp dans le d\xE9sert", de: "W\xFCstencamp" },
    "Sahara Trip": { no: "Sahara-tur", sv: "Sahararesa", fr: "Excursion au Sahara", de: "Sahara-Reise" },
    "Essaouira Day Trip": { no: "Essaouira dagstur", sv: "Essaouira dagstur", fr: "Excursion \xE0 Essaouira", de: "Essaouira Tagesausflug" },
    "Cooking Class": { no: "Matlagingskurs", sv: "Matlagningskurs", fr: "Cours de cuisine", de: "Kochkurs" },
    "Hot Air Balloon": { no: "Luftballong", sv: "Luftballong", fr: "Montgolfi\xE8re", de: "Hei\xDFluftballon" },
    "Paragliding": { no: "Paragliding", sv: "Sk\xE4rmflygning", fr: "Parapente", de: "Gleitschirmfliegen" },
    "Quad/Buggy": { no: "Quad/Buggy", sv: "Quad/Buggy", fr: "Quad/Buggy", de: "Quad/Buggy" },
    "Camel Ride": { no: "Kameltur", sv: "Kamelridning", fr: "Balade \xE0 dos de chameau", de: "Kamelritt" },
    "Horse Riding": { no: "Rideturer", sv: "Ridning", fr: "\xC9quitation", de: "Reiten" },
    "Jet Ski": { no: "Vannscooter", sv: "Vattenskoter", fr: "Jet ski", de: "Jetski" },
    "Surfing": { no: "Surfing", sv: "Surfing", fr: "Surf", de: "Surfen" },
    "Boat Trip": { no: "B\xE5ttur", sv: "B\xE5ttur", fr: "Excursion en bateau", de: "Bootsfahrt" },
    "Golf": { no: "Golf", sv: "Golf", fr: "Golf", de: "Golf" },
    "Waterfalls Trip": { no: "Fossetur", sv: "Vattenfallstur", fr: "Excursion aux cascades", de: "Wasserf\xE4lle-Ausflug" },
    ...
    "Breakfast": { no: "Frokost", sv: "Frukost", fr: "Petit-d\xE9jeuner", de: "Fr\xFChst\xFCck" },
    "Lunch": { no: "Lunsj", sv: "Lunch", fr: "D\xE9jeuner", de: "Mittagessen" },
    "Dinner": { no: "Middag", sv: "Middag", fr: "D\xEEner", de: "Abendessen" },
    "Show / Entertainment": { no: "Show", sv: "Underh\xE5llning", fr: "Spectacle", de: "Show" },
    "Spa/Hammam": { no: "Spa/Hammam", sv: "Spa/Hammam", fr: "Spa/Hammam", de: "Spa/Hammam" },
    "Massage": { no: "Massasje", sv: "Massage", fr: "Massage", de: "Massage" },
    "Pool Day": { no: "Bassengdag", sv: "Pooldag", fr: "Journ\xE9e piscine", de: "Pooltag" },
    "Free Time": { no: "Fritid", sv: "Fritid", fr: "Temps libre", de: "Freizeit" },
    "Other": { no: "Annet", sv: "\xD6vrigt", fr: "Autre", de: "Sonstiges" }
  };
  const BK_PALETTE = ["#e0432a", "#0a84ff", "#34c759", "#ff9f0a", "#af52de", "#ff2d55", "#0aa2c0", "#a2845e", "#d4a017", "#1c7a3f", "#5856d6", "#ff6482", "#00b8a3", "#c2410c"];
  function bkColor(b) {
    const s = String(b && (b.reference || b.id || b.client_name) || "");
    let n = 0;
    for (let i = 0; i < s.length; i++) n = n * 31 + s.charCodeAt(i) >>> 0;
    return BK_PALETTE[n % BK_PALETTE.length];
  }
  const BOOKING_KINDS = ["itinerary", "quickbook", "tweak"];
  function svg(children, vb) {
    return h("svg", { width: 18, height: 18, viewBox: vb || "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.8, strokeLinecap: "round", strokeLinejoin: "round" }, children);
  }
  const P = (d) => h("path", { d });
  const ICON = {
  ...
  function Login({ onAuthed, recoveryMode, onRecoveryDone, recoveryNotice }) {
    ...
    const pickMode = (m) => {
      ...
      setPass("");
      setNotice("");
      setEmail(m === "admin" ? adminHintEmail : PARTNER_HINT_EMAIL);
    };
    const forgot = async (e) => {
      ...
      else setNotice("Password-reset email sent to " + email.trim() + ". It will return to this admin page.");
    };
    const sendSecureLink = async (e) => {
      ...
      if (!secureEmail) {
        setErr("Enter your email first, then request the secure sign-in link.");
        return;
      }
      setBusy(true);
    ...
    const submit = async (e) => {
      ...
      const roleData = await ensureStaffAccess(signedInUser, typedEmail) || (normEmail(typedEmail) === normEmail(DEFAULT_ADMIN_EMAIL) ? "admin" : null);
      setBusy(false);
    ...
    if (recoveryMode) {
      return h("div", { className: "msa-login" }, h(
        "form",
        { className: "msa-login-card", onSubmit: submitRecovery },
        h("div", { className: "msa-login-logo-wrap" }, h("img", { src: "assets/logo.png", alt: "MarrakechStory", className: "msa-login-logo", onError: (e) => {
    ...
    return h("div", { className: "msa-login" }, h(
      "form",
      { className: "msa-login-card", onSubmit: submit },
      h("div", { className: "msa-login-logo-wrap" }, h("img", { src: "assets/logo.png", alt: "MarrakechStory", className: "msa-login-logo", onError: (e) => {
        e.target.style.display = "none";
      } })),
      ...
      h(
        "div",
        { className: "msa-login-modes" },
        h("button", { type: "button", className: "msa-login-mode" + (mode === "admin" ? " active" : ""), onClick: () => pickMode("admin") }, "\u{1F6E1}\uFE0F Admin"),
        h("button", { type: "button", className: "msa-login-mode" + (mode === "partner" ? " active" : ""), onClick: () => pickMode("partner") }, "\u{1F464} Partner")
      ),
      ...
      h(
        ...
        h("p", null, mode === "admin" ? "Use your admin password first. If that fails, send a one-time secure sign-in link to the same admin email without creating a new account or changing your password." : "Use the partner password first. If you are locked out, send a one-time secure sign-in link to the partner email on file."),
  ...
  function Donut({ segments, size }) {
    ...
    return h(
      "div",
      { className: "msa-donut-wrap" },
      h(
        "svg",
        { width: size, height: size, viewBox: "0 0 " + size + " " + size, className: "msa-donut" },
        h("circle", { cx, cy, r, fill: "none", stroke: "#eee", strokeWidth: 16 }),
        segments.map((s, i) => {
          const len = s.value / total * C;
          const el = h("circle", { key: i, cx, cy, r, fill: "none", stroke: s.color, strokeWidth: 16, strokeDasharray: len + " " + (C - len), strokeDashoffset: -off, transform: "rotate(-90 " + cx + " " + cy + ")" });
          off += len;
          return el;
        })
      ),
      h("div", { className: "msa-donut-mid" }, h("strong", null, Math.round(total / 1e3) + "k"), h("span", null, "total"))
    );
  }
  function Bars({ data, color }) {
    const max = Math.max(1, ...data.map((d) => d.value));
    return h("div", { className: "msa-bars" }, data.map((d, i) => h(
      "div",
      { key: i, className: "msa-bar-col" },
  ...
  function MonthCalendar({ bookings, sel, onSelect, compact, year }) {
    const [cursor, setCursor] = useState(() => {
      const d = sel ? new Date(sel) : /* @__PURE__ */ new Date();
    ...
    const dayMap = useMemo(() => {
      ...
      bookings.forEach((b) => {
        if (!b.arrival_date || !b.departure_date) {
          if (b.arrival_date) (m[b.arrival_date] = m[b.arrival_date] || []).push(b);
          return;
        }
        let d = new Date(b.arrival_date);
        const e = new Date(b.departure_date);
        let g = 0;
        while (d <= e && g++ < 400) {
    ...
    if (year) {
      const miniMonth = (mi) => {
        const first2 = mondayOffset(new Date(Y, mi, 1));
        const dim2 = new Date(Y, mi + 1, 0).getDate();
        const cells2 = [];
        CAL_DOW_MINI.forEach((d, i) => cells2.push(h("span", { key: "h" + i, className: "msa-yr-dow" }, d)));
        for (let i = 0; i < first2; i++) cells2.push(h("span", { key: "e" + i, className: "msa-yr-day empty" }));
      ...
      return h(
        ...
        h(
          "div",
          { className: "msa-card-head" },
          h("h3", null, String(Y)),
          h(
            "div",
            { className: "msa-cal-controls" },
            h("button", { className: "msa-icon-btn", onClick: () => setCursor(new Date(Y - 1, M, 1)) }, ICON.chevL()),
            h("button", { className: "msa-btn msa-btn-sm", onClick: () => {
              const d = /* @__PURE__ */ new Date();
    ...
    const first = mondayOffset(new Date(Y, M, 1));
    ...
    for (let d = 1; d <= dim; d++) {
      ...
      cells.push(h(
        ...
        cnt > 0 && h(
          "div",
          { className: "msa-cal-tip" },
          h("div", { className: "msa-cal-tip-date" }, fmtDate(k)),
          items.slice(0, 6).map((b, i) => {
            const a = bkDayActs(b, k);
            return h(
              "div",
              { key: i, className: "msa-cal-tip-row" },
    ...
    return h(
      ...
      h(
        ...
        h(
          "div",
          { className: "msa-cal-controls" },
          h("button", { className: "msa-icon-btn", onClick: () => setCursor(new Date(Y, M - 1, 1)) }, ICON.chevL()),
          h("button", { className: "msa-btn msa-btn-sm", onClick: () => {
            const d = /* @__PURE__ */ new Date();
            setCursor(new Date(d.getFullYear(), d.getMonth(), 1));
            onSelect && onSelect(todayISO());
          } }, "Today"),
  ...
  function TeamActivity() {
    ...
    const online = partner && now - new Date(partner.last_seen).getTime() < 12e4;
    const editingNow = audit.length && audit[0].actor_role === "partner" && now - new Date(audit[0].created_at).getTime() < 12e4;
    return h(
      "div",
      { className: "msa-card msa-team" },
      h(
        "div",
        { className: "msa-card-head" },
        h("h3", null, ICON.clients(), " Team activity"),
        h("span", { className: "msa-team-status " + (online ? "on" : "off") }, h("span", { className: "msa-team-dot" }), online ? "Partner connected" : "Partner offline")
      ),
      partner ? h(
        "div",
        { className: "msa-team-who" },
        h("strong", null, partner.name || partner.email),
        h("span", { className: "msa-dim" }, online ? "Active now \xB7 " + (partner.current_view || "console") : "Last seen " + (agoOf(partner.last_seen) || "?") + " ago")
      ) : h("div", { className: "msa-dim", style: { padding: "4px 0 10px" } }, "No partner account yet \u2014 set one up in Settings."),
      editingNow ? h("div", { className: "msa-team-live" }, "\u25CF " + (partner && partner.name || "Partner") + " is editing now: " + (audit[0].action || "") + " " + (audit[0].entity || "")) : null,
      h("div", { className: "msa-team-feed" }, audit.length === 0 ? h("div", { className: "msa-empty" }, "No changes logged yet.") : audit.map((a) => h(
        "div",
        { key: a.id, className: "msa-team-row" + (a.actor_role === "partner" ? " partner" : "") },
        h("span", { className: "msa-team-ago" }, agoOf(a.created_at)),
        h("span", { className: "msa-team-act" }, (a.actor_role === "partner" ? "\u{1F464} " : "\u{1F6E1}\uFE0F ") + (a.action || "") + " " + (a.entity || "") + (a.detail ? " \u2014 " + a.detail : ""))
      )))
    );
  }
  function Dashboard({ bookings, tasks, leads, clients, suppliers, go, openBooking, reload }) {
    ...
    const activeList = bookings.filter((b) => !b.archived && b.status !== "cancelled" && b.arrival_date && b.departure_date && dStr(b.arrival_date) <= T && T <= dStr(b.departure_date)).sort((a, b) => dStr(a.departure_date).localeCompare(dStr(b.departure_date)));
    ...
    const toggleTask = async (t, e) => {
      if (e) e.stopPropagation();
      const done = t.status === "completed";
      await dbUpdate("tasks", t.id, done ? { status: "pending", done_by: null, done_at: null } : { status: "completed", done_by: CURRENT_EMAIL, done_at: (/* @__PURE__ */ new Date()).toISOString() });
      logAudit(done ? "reopened task" : "completed task", "workspace", t.id, t.title);
      reload && reload();
    };
    ...
    const kpi = (label, value, cls, tab) => h(
      "button",
      { className: "msa-kpi " + cls, onClick: () => tab && go(tab) },
    ...
    return h(
      R.Fragment,
      null,
      addTask && h(TaskModal, { initial: {}, onClose: () => setAddTask(false), onSaved: () => {
        setAddTask(false);
        reload();
      } }),
      reqPop && h(
        "div",
        { className: "msa-modal-backdrop", onClick: () => setReqPop(false) },
        h(
          "div",
          { className: "msa-modal", onClick: (e) => e.stopPropagation() },
          h(
            "div",
            { className: "msa-modal-head" },
            h("h2", null, ICON.requests(), " Latest requests"),
            h(
              "div",
              null,
              h("button", { className: "msa-btn msa-btn-primary", onClick: () => {
                setReqPop(false);
                go("requests");
              } }, "Open Requests page"),
          ...
          h(
            "div",
            { className: "msa-modal-body" },
            leads.length === 0 ? h("div", { className: "msa-empty" }, "No website requests yet.") : leads.slice(0, 25).map((l) => h(
              "button",
              { key: l.id, className: "msa-line-item", style: { width: "100%" }, onClick: () => {
                setReqPop(false);
                go("requests");
              } },
              h(
                "div",
                null,
                h("strong", null, l.name || l.email || "Anonymous"),
                reqTitle(l) ? h("span", { className: "msa-text-brand", style: { marginLeft: 6, fontWeight: 600 } }, "\xB7 " + reqTitle(l)) : null,
                h("span", { className: "msa-dim" }, " \xB7 " + reqKindLabel(l))
              ),
      ...
      h(
        ...
        h(
          ...
          h(
            "div",
            { className: "msa-dash-side" },
            h(
              "div",
              { className: "msa-card msa-dash-box" },
              ...
              h("div", { className: "msa-dash-scroll" }, activeList.length === 0 && future.length === 0 ? h("div", { className: "msa-empty" }, "No active or upcoming bookings.") : h(
                "div",
                { className: "msa-remind-list" },
                activeList.map((b) => {
                  const n = daysUntil(dStr(b.departure_date));
                  const act = bookingActivity(b);
                  return h(
                    "button",
                    { key: "a" + b.id, className: "msa-remind msa-remind-live" + (act ? " msa-remind-glow" : ""), onClick: () => openBooking(b) },
                    h("div", { className: "msa-remind-cd msa-cd-live" }, h("strong", null, "\u25CF"), h("span", null, "now")),
                    h(
                      "div",
                      { className: "msa-remind-body" },
                      h("strong", null, b.client_name),
                      act ? h("span", { className: "msa-remind-act" }, act.label) : null,
                      h("span", { className: "msa-dim" }, "On trip \xB7 " + (n === 0 ? "ends today" : "ends in " + n + (n === 1 ? " day" : " days")) + " \xB7 " + ((b.adults || 0) + (b.kids || 0)) + " pax")
                    ),
                ...
                future.slice(0, 6).map((b) => {
                  const n = daysUntil(dStr(b.arrival_date));
                  const cd = n <= 30 ? "msa-cd-soon" : "msa-cd-far";
                  const act = bookingActivity(b);
                  return h(
                    "button",
                    { key: b.id, className: "msa-remind" + (act ? " msa-remind-glow" : ""), onClick: () => openBooking(b) },
                    ...
                    h(
                      ...
                      h("strong", null, b.client_name),
                      act ? h("span", { className: "msa-remind-act" }, act.label) : null,
                      h("span", { className: "msa-dim" }, (b.arrival_city || "") + " \u2192 " + (b.departure_city || "") + " \xB7 " + (b.total_days || "?") + "D \xB7 " + ((b.adults || 0) + (b.kids || 0)) + " pax")
                    ),
            ...
            h(
              ...
              h(
                "div",
                { className: "msa-card-head" },
                h("h3", null, ICON.tasks(), " Workspace"),
                h(
                  "div",
              ...
              h("div", { className: "msa-dash-scroll" }, openTasks.length === 0 ? h("div", { className: "msa-empty" }, "No shared tasks. Add one in the Workspace.") : openTasks.slice(0, 6).map((t) => {
                const n = daysUntil((t.due || "").slice(0, 10));
                const overdue = n != null && n < 0;
                const soon = n != null && n >= 0 && n <= 2;
                return h(
                  "div",
                  { key: t.id, className: "msa-task-mini" },
                  h("button", { className: "msa-check msa-check-sm", title: "Mark done", onClick: (e) => toggleTask(t, e) }, ""),
                  h("div", { className: "msa-task-mini-body", onClick: () => go("tasks"), style: { cursor: "pointer" } }, h("span", null, t.title), h("span", { className: "msa-dim " + (overdue ? "msa-text-red" : soon ? "msa-text-orange" : "") }, (t.due ? fmtDate(t.due) : "No due") + (overdue ? " \xB7 Overdue" : soon ? " \xB7 Due soon" : ""))),
  ...
  function BookingModal({ initial, onClose, onSaved, onView, suppliers }) {
    ...
    const loadThread = useCallback(() => {
      const sb = getSB();
      if (!sb || !initial || !initial.email) return;
      sb.from("messages").select("*").eq("client_email", initial.email).order("created_at", { ascending: true }).then(({ data }) => {
        setThread(data || []);
        const unread = (data || []).filter((x) => x.sender === "client" && !x.read_by_admin).map((x) => x.id);
        if (unread.length) sb.from("messages").update({ read_by_admin: true }).in("id", unread);
      });
    }, [initial && initial.id]);
    useEffect(() => {
      loadThread();
    ...
    const sendReply = async () => {
      const sb = getSB();
      if (!reply.trim() || !sb || !initial || !initial.email) return;
      const body = reply.trim();
      setReply("");
      await sb.from("messages").insert({ client_email: initial.email, sender: "admin", body, booking_id: initial.id, read_by_admin: true });
      loadThread();
    };
    ...
    const loadFiles = useCallback(() => {
      const sb = getSB();
      if (!sb || !initial || !initial.id) {
        setFiles([]);
        return;
      }
      sb.storage.from(FILE_BUCKET).list(String(initial.id), { limit: 200, sortBy: { column: "created_at", order: "desc" } }).then(({ data }) => setFiles((data || []).filter((f) => f.name !== ".emptyFolderPlaceholder")));
    }, [initial && initial.id]);
    useEffect(() => {
      loadFiles();
    }, [loadFiles]);
    const uploadFiles = async (fileList) => {
      const sb = getSB();
      if (!sb || !initial || !initial.id) {
        alert("Save the booking first, then you can upload files to it.");
        return;
      }
      ...
      for (const file of list) {
        ...
        const { error } = await sb.storage.from(FILE_BUCKET).upload(path, file, { upsert: false, cacheControl: "3600", contentType: file.type || void 0 });
        if (error) {
    ...
    const delFile = async (name) => {
      const sb = getSB();
      if (!sb || !initial || !initial.id) return;
    ...
    const setDate = (k, v) => setB((p) => {
      ...
      if (arr && dep) {
        const nights = Math.max(0, Math.round((new Date(dep) - new Date(arr)) / 864e5));
    ...
    const recalcCosts = (lines) => {
      const sum = (cat) => lines.filter((l) => l.cat === cat).reduce((s, l) => s + (+l.amount || 0), 0);
      const ct = sum("transport"), ca = sum("accommodation"), cv = sum("activities");
      return { cost_lines: lines, cost_transportation: ct, cost_accommodation: ca, cost_activities: cv, total_cost: ct + ca + cv };
    };
    const addLine = (cat) => setB((p) => {
      const nl = [...p.cost_lines || [], { cat, collab: "", amount: 0 }];
      return { ...p, ...recalcCosts(nl) };
    });
    const setLine = (idx, key, val) => setB((p) => {
      const nl = (p.cost_lines || []).map((l, i) => i === idx ? { ...l, [key]: val } : l);
      return { ...p, ...recalcCosts(nl) };
    });
    const delLine = (idx) => setB((p) => {
      const nl = (p.cost_lines || []).filter((_, i) => i !== idx);
      return { ...p, ...recalcCosts(nl) };
    });
    ...
    const addCollabLine = async (idx, defType) => {
      const name = (window.prompt("New collaborator name:") || "").trim();
      if (!name) return;
      const res = await dbInsert("suppliers", { name, type: defType || "activity" });
      if (res.error) {
        alert("Could not add: " + res.error.message);
        return;
      }
      const row = res.data && res.data[0];
      if (!row) return;
      setExtraSups((p) => [...p, row]);
      setLine(idx, "collab", row.id);
    };
    const sc = normalizeCurrency(b.sell_currency), cc = normalizeCurrency(b.cost_currency, "MAD");
    const fx = parseFloat(b.fx_rate) || 0;
    const sameCur = sc === cc;
    ...
    const setDay = (i, k, v) => setB((p) => {
      const a = [...p.daily_itinerary];
      a[i] = { ...a[i], [k]: v };
      return { ...p, daily_itinerary: a };
    });
    const addAct = (i) => setB((p) => {
      const a = [...p.daily_itinerary];
      a[i] = { ...a[i], activities: [...a[i].activities || [], { time: "09:00", type: "Transport", details: "", confirmed: false }] };
      return { ...p, daily_itinerary: a };
    });
    const setAct = (di, ai, k, v) => setB((p) => {
      const a = [...p.daily_itinerary];
      const ac = [...a[di].activities];
      ac[ai] = { ...ac[ai], [k]: v };
      a[di] = { ...a[di], activities: ac };
      return { ...p, daily_itinerary: a };
    });
    const delAct = (di, ai) => setB((p) => {
      const a = [...p.daily_itinerary];
      const ac = [...a[di].activities];
      ac.splice(ai, 1);
      a[di] = { ...a[di], activities: ac };
    ...
    const buildCollab = (kind) => {
      ...
      if (kind === "transport") return [
        ...
        "Arrival: " + (b.arrival_city || "Marrakech") + " \xB7 " + (b.arrival_date || "?") + " \xB7 flight & time: ______",
        "Departure: " + (b.departure_city || "Marrakech") + " \xB7 " + (b.departure_date || "?") + " \xB7 flight & time: ______"
      ].join("\n");
      return [
    ...
    const copyCollab = (kind) => {
      try {
        navigator.clipboard.writeText(collabTx[kind] || "");
        setCollabCopied(kind);
        setTimeout(() => setCollabCopied(""), 2e3);
      } catch (e) {
      }
    };
    ...
    const save = async (closeAfter) => {
      if (!b.client_name.trim()) {
        alert("Client name is required");
        return;
      }
      setBusy(true);
      const row = { ...b, daily_itinerary: normalizeItinerary(b.daily_itinerary || b.itinerary, b.arrival_date), reference: b.reference || "MS-" + Math.random().toString(36).slice(2, 8).toUpperCase(), travelers: (+b.adults || 0) + (+b.kids || 0), sell_currency: normalizeCurrency(b.sell_currency), cost_currency: normalizeCurrency(b.cost_currency, "MAD"), updated_at: (/* @__PURE__ */ new Date()).toISOString() };
      ...
      ["collab_transport", "collab_accommodation", "collab_activities"].forEach((k) => {
        if (!row[k]) row[k] = null;
      });
    ...
    return h("div", { className: "msa-modal-backdrop", onClick: onClose }, h(
      "div",
      { className: "msa-modal msa-modal-wide msa-modal-booking", onClick: (e) => e.stopPropagation() },
      h(
        "div",
        { className: "msa-modal-head" },
        h(
          "div",
          { className: "msa-bk-title" },
      ...
      h(
        ...
        h("h4", { className: "msa-section" }, "Client accommodation \xB7 operational location"),
        ...
        +b.kids > 0 && h(
          "div",
          { className: "msa-field msa-field-wide" },
          h("label", null, "Age of each child"),
          h("div", { className: "msa-kid-ages" }, Array.from({ length: +b.kids || 0 }).map((_, i) => h(
            "label",
            { key: i, className: "msa-kid-age" },
        ...
        h("div", { className: "msa-day-grid" }, (b.daily_itinerary || []).map((day, di) => h(
          "div",
          { key: di, className: "msa-day-card" },
          ...
          (day.activities || []).map((a, ai) => {
            ...
            return h(
              "div",
              { key: ai, className: "msa-act" },
              h(
                "div",
                { className: "msa-act-row" },
              ...
              h(
                "div",
                { className: "msa-grid-2 mso-act-fields" },
                ...
                h("input", { type: "number", step: "any", placeholder: "Latitude", value: (_a = a.latitude) != null ? _a : "", onChange: (e) => setAct(di, ai, "latitude", e.target.value) }),
                h("input", { type: "number", step: "any", placeholder: "Longitude", value: (_b = a.longitude) != null ? _b : "", onChange: (e) => setAct(di, ai, "longitude", e.target.value) }),
                ...
                h("input", { type: "number", step: "any", placeholder: "Pickup latitude", value: (_c = a.pickup_latitude) != null ? _c : "", onChange: (e) => setAct(di, ai, "pickup_latitude", e.target.value) }),
                h("input", { type: "number", step: "any", placeholder: "Pickup longitude", value: (_d = a.pickup_longitude) != null ? _d : "", onChange: (e) => setAct(di, ai, "pickup_longitude", e.target.value) }),
                h("input", { type: "number", step: "any", placeholder: "Drop-off latitude", value: (_e = a.dropoff_latitude) != null ? _e : "", onChange: (e) => setAct(di, ai, "dropoff_latitude", e.target.value) }),
                h("input", { type: "number", step: "any", placeholder: "Drop-off longitude", value: (_f = a.dropoff_longitude) != null ? _f : "", onChange: (e) => setAct(di, ai, "dropoff_longitude", e.target.value) }),
        ...
        h(
          "h4",
          { className: "msa-section msa-section-row" },
          h("span", null, "Files & documents"),
          initial && initial.id ? h(
            "label",
            { className: "msa-btn msa-btn-sm msa-btn-primary msa-upload-label" },
            ICON.plus(),
            upBusy ? "Uploading\u2026" : "Upload",
            h("input", { type: "file", multiple: true, disabled: upBusy, style: { display: "none" }, onChange: (e) => {
        ...
        initial && initial.id ? h(
          "div",
          { className: "msa-files" },
          files.length === 0 ? h("div", { className: "msa-dim", style: { padding: "6px 2px" } }, "No files yet \u2014 upload contracts, vouchers, passports, PDFs or photos from your computer.") : files.map((f) => h(
            "div",
            { key: f.name, className: "msa-file-row" },
            h("a", { className: "msa-file-name", href: "#", onClick: (e) => {
              e.preventDefault();
              openFile(f.name);
            } }, ICON.doc(), h("span", null, niceName(f.name))),
            h("span", { className: "msa-file-size" }, fmtSize(f.metadata && f.metadata.size)),
            h("button", { className: "msa-icon-btn", title: "Delete file", onClick: () => delFile(f.name) }, ICON.trash())
          ))
        ) : null,
        ...
        isAdminRole() ? function() {
          ...
          const catBlock = (cat, label, defType) => {
            const items = lines.map((l, i) => ({ l, i })).filter((x) => x.l.cat === cat);
            return h(
              "div",
              { className: "msa-field msa-field-wide msa-cost-cat" },
              h("label", null, label + " \u2014 " + money(b["cost_" + (cat === "transport" ? "transportation" : cat)], cc)),
              items.map(({ l, i }) => h(
                "div",
                { key: i, className: "msa-costline-row" },
                ...
                h(
                  "select",
                  { className: "msa-costline-sel", value: l.collab || "", onChange: (e) => setLine(i, "collab", e.target.value) },
                  h("option", { value: "" }, "Collaborator\u2026"),
                  allSups.map((s) => h("option", { key: s.id, value: s.id }, s.name))
                ),
                h("button", { className: "msa-icon-btn msa-collab-add", title: "Create a new collaborator", onClick: () => setCollabModal({ lineIdx: i, defType }) }, ICON.plus()),
          ...
          return h(
            ...
            sameCur ? h("div", { className: "msa-field" }, h("label", null, "Exchange rate"), h("div", { className: "msa-readout msa-dim" }, "Same currency \u2014 no conversion")) : h("div", { className: "msa-field" }, h("label", null, "1 " + cc + " = ? " + sc), h("input", { type: "number", step: "0.0001", placeholder: "e.g. 0.95", value: b.fx_rate == null ? "" : b.fx_rate, onChange: (e) => set("fx_rate", e.target.value) })),
        ...
        isAdminRole() ? h(
          ...
          h(
            "div",
            { className: "msa-field" },
            h("label", null, "Deposit"),
            h(
              "label",
              { className: "msa-toggle" },
              ...
              h("span", { className: "msa-toggle-lbl" }, b.deposit_enabled !== false ? "Required \xB7 " + money(b.deposit_amount || 0, sc) : "Not required")
            )
          ),
        ...
        h(
          "div",
          { className: "msa-collab-send" },
          h(
            "div",
            { className: "msa-collab-box" },
            h(
              "div",
              { className: "msa-collab-box-head" },
              h("span", { className: "msa-collab-box-t" }, ICON.van ? ICON.van() : null, "\u{1F697} Driver / transport"),
          ...
          h(
            ...
            h(
              "div",
              { className: "msa-collab-box-head" },
              h("span", { className: "msa-collab-box-t" }, "\u{1F3E8} Accommodation"),
        ...
        initial && initial.id && initial.email ? h(
          "div",
          null,
          h("h4", { className: "msa-section" }, "Messages with client"),
          h(
            "div",
            { className: "msa-msg-thread" },
            thread.length === 0 ? h("div", { className: "msa-dim", style: { padding: "8px 0" } }, "No messages yet.") : thread.map((m) => h(
              "div",
              { key: m.id, className: "msa-msg " + (m.sender === "admin" ? "me" : "them") },
              h("div", { className: "msa-msg-bubble" }, m.body),
              h("div", { className: "msa-msg-meta" }, (m.sender === "admin" ? "You" : b.client_name || "Client") + " \xB7 " + fmtDateTime(m.created_at))
            ))
          ),
          h(
            "div",
            { className: "msa-msg-composer" },
            h("input", { value: reply, placeholder: "Reply to the client\u2026", onChange: (e) => setReply(e.target.value), onKeyDown: (e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                sendReply();
              }
            } }),
            h("button", { className: "msa-btn msa-btn-primary", onClick: sendReply, disabled: !reply.trim() }, "Send")
          )
        ) : null
      ),
      collabModal !== null ? h(CollaboratorModal, { initial: { type: collabModal.defType }, onClose: () => setCollabModal(null), onSaved: (savedRow) => {
        if (savedRow && savedRow.id) {
          setExtraSups((p) => [...p, savedRow]);
          if (collabModal.lineIdx != null) setLine(collabModal.lineIdx, "collab", savedRow.id);
        }
        setCollabModal(null);
      } }) : null
    ));
  ...
  function exportPDF(filename, srcId) {
    const src = document.getElementById(srcId || "msa-print-both");
    if (!src) return;
    if (!window.html2pdf) {
      window.print();
      return;
    ...
    const reveal = () => {
      ...
      src.style.cssText = "position:fixed; top:0; left:0; width:" + PAGEW + "px; background:#fff; z-index:2147483000;";
    };
    const restore = () => {
      src.style.cssText = prevCss;
      try {
        cover.remove();
      } catch (e) {
      }
    };
    pages.forEach((p) => p.querySelectorAll("details").forEach((d) => {
      d.open = true;
    }));
    const captureOne = (el) => new Promise((resolve) => {
      window.html2pdf().set({ html2canvas: { scale: SCALE, useCORS: true, backgroundColor: "#ffffff", scrollX: 0, scrollY: -window.scrollY } }).from(el).toCanvas().get("canvas", (c) => resolve(c)).then(() => {
      }, () => resolve(null));
    });
    const waitReady = async () => {
      ...
      await Promise.all(imgs.map((im) => im.complete && im.naturalWidth ? null : new Promise((r) => {
    ...
    setTimeout(async () => {
      ...
      try {
        for (const el of pages) {
          const c = await captureOne(el);
          if (c) cropped.push(cropToContent(c));
        }
      } finally {
        restore();
      }
      ...
      tiny.style.cssText = "width:1px;height:1px;";
      document.body.appendChild(tiny);
      window.html2pdf().set({ jsPDF: { unit: "mm", format: "a4", orientation: "portrait" }, html2canvas: { scale: 1 }, margin: 0 }).from(tiny).toPdf().get("pdf", (pdf) => {
        try {
          tiny.remove();
        } catch (e) {
        }
  ...
  async function buildBookingPdf(b, S, opts) {
    ...
    const header = (kicker, title) => {
      y = 52;
      if (logo) {
        const lw = 42, lh = lw * logo.h / logo.w;
        doc.addImage(logo.d, "PNG", W - MX - lw, y - 16, lw, lh);
      }
      setF(8.5, "bold", RED);
      T(String(kicker).toUpperCase(), MX, y);
      y += 21;
      setF(24, "bold", INK);
      T(title, MX, y);
      y += 9;
    ...
    const infoRow = (items) => {
      ...
      items.forEach((it, i) => {
        const x = MX + i * colW;
        setF(7.5, "bold", DIM);
        T(String(it[0]).toUpperCase(), x, y);
        setF(9.5, "bold", INK);
        const ls = split(it[1] == null || it[1] === "" ? "\u2014" : it[1], colW - 8);
    ...
    const foot = () => {
      ...
      draw(LN);
      doc.setLineWidth(1);
      doc.line(MX, fy - 12, W - MX, fy - 12);
      setF(9, "normal", DIM);
      T(lang === "no" ? S_.invoice_footer || tr("foot_thanks") : tr("foot_thanks"), MX, fy);
      T(cWeb + "  |  " + cPhone, W - MX, fy, { align: "right" });
    };
    header(tr("itin_kicker"), tr("itin_title"));
    infoRow([
      [tr("for"), b.client_name],
      [tr("trip"), (b.arrival_city || "") + " -> " + (b.departure_city || "")],
      [tr("dates"), fmtDate(b.arrival_date) + " - " + fmtDate(b.departure_date)],
      [tr("travelers"), (b.adults || 0) + (b.kids || 0) + " " + tr("persons")],
      [tr("ref"), b.reference]
    ]);
    ...
    days.forEach((day, i) => {
      ...
      const titleStr = tr("day") + " " + (day.day || i + 1) + (day.date ? " \xB7 " + fmtDate(day.date) : "") + (day.city ? " \xB7 " + day.city : "");
      setF(11, "bold", INK);
      const tLines = split(titleStr, CW - 26);
      ensure(6 + tLines.length * 14 + acts.length * 20 + 14);
      fill(RED);
      doc.circle(MX + 7, y - 3, 8, "F");
      setF(9, "bold", [255, 255, 255]);
      T(String(day.day || i + 1), MX + 7, y - 3, { align: "center", baseline: "middle" });
      setF(11, "bold", INK);
      T(tLines, MX + 24, y);
      y += tLines.length * 14 + 2;
      acts.forEach((a) => {
        const dLines = a.details ? split(a.details, CW - 78) : [];
        ensure(14 + dLines.length * 11);
        setF(9, "bold", RED);
        T(String(a.time || ""), MX + 24, y);
        setF(9.5, "bold", INK);
        T(trAct(a.type || ""), MX + 78, y);
        if (dLines.length) {
          setF(9, "normal", DIM);
          T(dLines, MX + 78, y + 12);
          y += 12 + (dLines.length - 1) * 11;
        }
        y += 16;
      });
      y += 10;
    ...
    if (inc.length || exc.length) {
      ...
      setF(9.5, "bold", INK);
      if (inc.length) T(tr("included"), MX, y);
      if (exc.length) T(tr("not_included"), MX + CW / 2, y);
      y += 15;
      let yL = y, yR = y;
      setF(9, "normal", GRN);
      incL.forEach((ls) => {
    ...
    infoRow([
      [tr("billed_to"), b.client_name],
      [tr("invoice_no"), (S_.invoice_prefix || "INV") + "-" + (b.reference || "").split("-").pop()],
      [tr("date"), (/* @__PURE__ */ new Date()).toLocaleDateString(DOC_LOCALE[lang] || void 0)],
      [tr("method"), method],
      [tr("status"), STATUS_LABEL[b.status] || b.status]
    ]);
    if (b.address || b.email || b.phone) {
      setF(8, "bold", DIM);
      T(tr("bill_to").toUpperCase(), MX, y);
      y += 14;
      setF(12, "bold", INK);
      T(String(b.client_name || ""), MX, y);
      y += 14;
      setF(9, "normal", DIM);
      if (b.address) {
        const al = split(b.address, CW);
        T(al, MX, y);
        y += al.length * 12;
      }
      const cp = [b.email, b.phone].filter(Boolean).join("   \xB7   ");
      if (cp) {
        T(cp, MX, y);
        y += 12;
      }
    ...
    setF(8, "bold", DIM);
    T(tr("description").toUpperCase(), MX, y);
    T(tr("amount").toUpperCase(), W - MX, y, { align: "right" });
    y += 6;
    draw(LN);
    doc.setLineWidth(1);
    doc.line(MX, y, W - MX, y);
    ...
    setF(11, "bold", INK);
    T(tr("package"), MX, y);
    T(m(sub), W - MX, y, { align: "right" });
    y += 14;
    setF(9, "normal", DIM);
    T((b.total_nights || 0) + " " + tr("nights") + ": " + (b.arrival_city || "") + " -> " + (b.departure_city || ""), MX, y);
    y += 12;
    T((b.adults || 0) + (b.kids || 0) + " " + tr("travelers_word"), MX, y);
    y += 24;
    ...
    const totRow = (label, val, c, bold) => {
      setF(bold ? 11 : 10, bold ? "bold" : "normal", bold ? INK : DIM);
      T(label, tLX, y);
      setF(bold ? 12 : 10, bold ? "bold" : "normal", c || INK);
      T(val, tVX, y, { align: "right" });
      y += 16;
    };
    ...
    setF(9, "normal", DIM);
    const nl = split(payNote, CW - 20);
    const nh = nl.length * 12 + 16;
    fill([247, 247, 245]);
    doc.roundedRect(MX, y - 2, CW, nh, 4, 4, "F");
    setF(9, "normal", [70, 70, 76]);
    T(nl, MX + 10, y + 12);
    y += nh + 18;
    const isBank = /bank|transfer|virement/i.test(method);
    const bank = isBank ? [[tr("bank_name"), S_.bank_name || "BMCE Bank of Africa"], [tr("account_name"), S_.account_name || cName], ["RIB:", S_.rib || "011 450 0000 123456789012 34"], ["SWIFT:", S_.swift || "BMCE MAMC"]] : [[method + ":", S_.payment_info || S_.company_email || cPhone]];
    const boxH = 30 + bank.length * 14;
    ensure(boxH + 10);
    draw(LN);
    doc.setLineWidth(1);
    doc.roundedRect(MX, y, CW, boxH, 6, 6, "S");
    let by = y + 20;
    setF(14, "bold", INK);
    T(isBank ? tr("bank_details") : tr("pay_via") + " " + method, MX + 14, by);
    by += 18;
    bank.forEach((row) => {
      setF(9, "bold", DIM);
      T(String(row[0]), MX + 14, by);
      setF(9, "normal", INK);
      T(String(row[1]), MX + 130, by);
      by += 14;
    });
    y += boxH + 16;
    if (S_.terms_conditions) {
      ...
      y += 14;
      setF(8.5, "normal", DIM);
      const tl = split(S_.terms_conditions, CW);
      tl.forEach((ln) => {
        ensure(11);
        T(ln, MX, y);
        y += 11;
    ...
    if (stamp) {
      const sw = 118, sh = sw * stamp.h / stamp.w;
      const sy = Math.min(y + 6, BOTTOM - sh);
      doc.addImage(stamp.d, "PNG", W - MX - sw, sy, sw, sh);
    }
    foot();
    return doc;
  }
  function DocModal({ booking, initialType, onClose, settings, onEdit }) {
    ...
    const loadDocFiles = useCallback(() => {
      ...
      sb.storage.from(DOC_BUCKET).list(String(b.id), { limit: 200, sortBy: { column: "created_at", order: "desc" } }).then(({ data }) => setDocFiles((data || []).filter((f) => f.name !== ".emptyFolderPlaceholder")));
    }, [b && b.id]);
    useEffect(() => {
      loadDocFiles();
    }, [loadDocFiles]);
    const uploadDocs = async (fileList) => {
      const sb = getSB();
      if (!sb || !b || !b.id) {
        alert("This booking must be saved before attaching documents.");
        return;
      }
      const list = Array.from(fileList || []);
      if (!list.length) return;
      ...
      for (const file of list) {
        ...
        const { error } = await sb.storage.from(DOC_BUCKET).upload(path, file, { upsert: false, cacheControl: "3600", contentType: file.type || void 0 });
    ...
    const docBox = () => h(
      "div",
      { className: "msa-doc-docs msa-print-hide" },
      h("h3", null, "Documents"),
      h("p", { className: "msa-dim msa-doc-docs-hint" }, "Vouchers, tickets, passports, contracts \u2014 any file. Click to open anytime."),
      docFiles.length === 0 ? h("div", { className: "msa-docbox-empty" }, "No documents attached yet. Use \u201CUpload\u201D at the top to add files from your computer.") : h("div", { className: "msa-docbox" }, docFiles.map((f) => h(
        "div",
        { key: f.name, className: "msa-docbox-item" },
        h("a", { className: "msa-docbox-link", href: "#", onClick: (e) => {
          e.preventDefault();
          openDoc(f.name);
        } }, ICON.doc(), h("span", null, docNice(f.name))),
    ...
    const logo = h("img", { src: "assets/logo.png", alt: "", className: "msa-doc-logo", crossOrigin: "anonymous", onError: (e) => {
      e.target.style.display = "none";
    } });
    const itinerary = () => h(
      "div",
      { className: "msa-doc msa-doc-itin" },
      // Decorative travel-magazine header band
      h(
        "div",
        { className: "msa-itin-hero" },
        h("img", { src: "assets/logo.png", alt: "", className: "msa-itin-logo", crossOrigin: "anonymous", onError: (e) => {
          e.target.style.display = "none";
        } }),
      ...
      h(
        ...
        h("div", { className: "msa-itin-info-cell" }, h("span", { className: "msa-itin-info-k" }, t("travelers")), h("span", { className: "msa-itin-info-v" }, (b.adults || 0) + (b.kids || 0) + " " + t("persons") + " \xB7 " + (b.total_nights || 0) + " " + t("nights"))),
      ...
      function() {
        ...
        const renderDay = (day, i) => h(
          "div",
          { key: i, className: "msa-doc-day" },
          h("span", { className: "msa-doc-daynum", "aria-hidden": "true" }, day.day),
          h(
            "div",
            { className: "msa-doc-day-card" },
            h(
              "div",
              { className: "msa-doc-day-head" },
              h(
                "h3",
                null,
                t("day") + " " + day.day,
                day.date ? h("span", { className: "msa-doc-day-date" }, " \xB7 " + fmtDDMMYY(day.date)) : null,
                day.city ? " \xB7 " + day.city : ""
              )
            ),
        ...
        return h(
          "div",
          { className: "msa-doc-days msa-doc-days-2" },
      ...
      function() {
        const inc = (b.included || []).filter((x) => x && String(x).trim());
        const exc = (b.excluded || []).filter((x) => x && String(x).trim());
        return inc.length || exc.length ? h(
          "div",
          { className: "msa-doc-incl" },
          ...
          exc.length ? h("div", { className: "msa-incl-col" }, h("h3", null, t("not_included")), h("ul", { className: "msa-incl-list" }, exc.map((x, i) => h("li", { key: i, className: "msa-incl-no" }, x)))) : null
        ) : null;
      }(),
      docBox(),
    ...
    const invoice = () => {
      ...
      const dep = depEnabled ? Math.round(sub * depPct / 100) : 0;
      const restToPay = sub - dep;
      const balanceHighlight = paid > 0 ? sub - paid : restToPay;
      const method = b.payment_method || "Bank Transfer";
      const isBank = /bank|transfer|virement/i.test(method);
      const isCash = /cash/i.test(method);
      const payRow = (k, v) => [h("span", { className: "msa-dim" }, k), h("span", null, v || "\u2014")];
      const paymentBox = isBank ? h("div", { className: "msa-doc-bank" }, h("h3", null, t("bank_details")), h(
        "div",
        { className: "msa-bank-grid" },
        payRow(t("bank_name"), S.bank_name || "BMCE Bank of Africa"),
        payRow(t("account_name"), S.account_name || cName),
        payRow("RIB:", S.rib || "011 450 0000 123456789012 34"),
        payRow("SWIFT:", S.swift || "BMCE MAMC")
      )) : isCash ? h("div", { className: "msa-doc-bank" }, h("h3", null, t("cash_title")), h(
        "div",
        { className: "msa-bank-grid" },
        payRow(t("label_method"), t("cash_word")),
        payRow(t("label_when"), t("cash_when"))
      )) : h("div", { className: "msa-doc-bank" }, h("h3", null, t("pay_via") + " " + method), h(
        "div",
        { className: "msa-bank-grid" },
        payRow(method + ":", method === "Revolut" ? S.revolut || S.company_phone || cPhone : method === "Wise" ? S.wise || S.company_email : method === "PayPal" ? S.paypal || S.company_email : S.payment_info || S.company_email)
      ));
      const payNote = depEnabled ? tFmt("paynote_deposit", { dep: m(dep), pct: depPct, rest: m(sub - dep) }) : tFmt("paynote_full", { sub: m(sub) });
      ...
      return h(
        "div",
        { className: "msa-doc msa-doc-itin msa-doc-inv" },
        h(
          "div",
          { className: "msa-itin-hero" },
          h("img", { src: "assets/logo.png", alt: "", className: "msa-itin-logo", crossOrigin: "anonymous", onError: (e) => {
        ...
        h(
          ...
          infoCell(t("date"), (/* @__PURE__ */ new Date()).toLocaleDateString(DOC_LOCALE[lang] || void 0)),
          infoCell(t("method"), method),
          infoCell(t("status"), STATUS_LABEL[b.status] || b.status)
        ),
        h(
          "div",
          { className: "msa-inv-body" },
          b.address || b.email || b.phone ? h("div", { className: "msa-inv-billto" }, h("h3", null, t("bill_to")), h("p", { className: "msa-doc-big" }, b.client_name), b.address ? h("p", { className: "msa-dim" }, b.address) : null, [b.email, b.phone].filter(Boolean).length ? h("p", { className: "msa-dim" }, [b.email, b.phone].filter(Boolean).join("   \xB7   ")) : null) : null,
          h(
            "table",
            { className: "msa-table msa-doc-table" },
            h("thead", null, h("tr", null, h("th", null, t("description")), h("th", { className: "msa-right" }, t("amount")))),
          ...
          h("div", { className: "msa-doc-sign" }, h("img", { src: "assets/stamp.png", alt: "", className: "msa-doc-stamp", crossOrigin: "anonymous", onError: (e) => {
            e.target.style.display = "none";
          } }))
        ),
        h("div", { className: "msa-doc-foot" }, h("p", null, lang === "no" ? S.invoice_footer || t("foot_thanks") : t("foot_thanks")), h("p", null, cWeb + " | " + cPhone))
    ...
    const doPrint = () => {
      try {
        document.querySelectorAll("#msa-print-both details").forEach((d) => {
          d.open = true;
        });
      } catch (e) {
      }
      ...
      const cleanup = () => {
        if (cleaned) return;
        cleaned = true;
        document.body.classList.remove("ms-print-doc");
    ...
    return h(
      "div",
      { className: "msa-modal-backdrop", onClick: onClose },
      h(
        "div",
        { className: "msa-modal msa-modal-doc", onClick: (e) => e.stopPropagation() },
        h(
          "div",
          { className: "msa-modal-head msa-print-hide msa-doc-topbar" },
          h(
            "div",
            { className: "msa-doc-head-left" },
            ...
            h(
              "label",
              { className: "msa-doc-pick", title: "Document language" },
            ...
            type === "invoice" && isAdminRole() ? h(
              "label",
              { className: "msa-doc-pick", title: "Invoice currency" },
              h("select", { value: curr, onChange: (e) => setCurr(e.target.value) }, CURRENCIES.map((c) => h("option", { key: c, value: c }, c)))
            ) : null,
          ...
          h(
            "div",
            { className: "msa-doc-head-right" },
            h(
              "label",
              { className: "msa-btn msa-btn-sm msa-upload-label", title: "Upload documents from your computer" },
              ICON.pdf(),
              docUpBusy ? "Uploading\u2026" : "Upload",
              h("input", { type: "file", multiple: true, disabled: docUpBusy, style: { display: "none" }, onChange: (e) => {
        ...
        h(
          ...
          h(
            "div",
            { id: "msa-print-both", className: "msa-print-both", "aria-hidden": "true" },
  ...
  function Bookings({ bookings, reload, settings, focusBooking, clearFocus, suppliers }) {
    ...
    const [sort, setSort] = useState({ k: "arrival_date", d: "asc" });
    ...
    const matchB = (b) => {
      ...
      if (!q) return true;
      return [b.client_name, b.reference, b.email, b.phone, b.arrival_date, b.departure_date, b.selling_price, b.total_cost, b.arrival_city].join(" ").toLowerCase().includes(q.toLowerCase());
    };
    const val = (b, k) => k === "profit" ? (+b.selling_price || 0) - (+b.total_cost || 0) : k === "travelers" ? (+b.adults || 0) + (+b.kids || 0) : b[k];
    const sorter = (a, b) => {
      let x = val(a, sort.k), y = val(b, sort.k);
      if (typeof x === "string") x = x.toLowerCase();
      if (typeof y === "string") y = y.toLowerCase();
      if (x == null) x = "";
      if (y == null) y = "";
      if (x < y) return sort.d === "asc" ? -1 : 1;
      if (x > y) return sort.d === "asc" ? 1 : -1;
      return 0;
    };
    ...
    const archiveB = async (b, e) => {
      e.stopPropagation();
      await dbUpdate("bookings", b.id, { archived: !b.archived });
      reload();
    };
    const del = async (b, e) => {
      e.stopPropagation();
      if (confirm("Delete " + (b.reference || b.client_name) + "?")) {
    ...
    const exportCSV = () => {
      ...
      const head = ["Reference", "Client", "Email", "Phone", "Arrival", "Departure", "Travelers", "Price", "Cost", "Profit", "Status"];
      const esc = (s) => '"' + String(s == null ? "" : s).replace(/"/g, '""') + '"';
      const lines = [head.join(",")];
      rows.forEach((b) => lines.push([b.reference, b.client_name, b.email, b.phone, b.arrival_date, b.departure_date, (+b.adults || 0) + (+b.kids || 0), b.selling_price, b.total_cost, (+b.selling_price || 0) - (+b.total_cost || 0), STATUS_LABEL[b.status]].map(esc).join(",")));
      const blob = new Blob([lines.join("\n")], { type: "text/csv" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "marrakechstory-bookings.csv";
      a.click();
      URL.revokeObjectURL(url);
    };
    ...
    const row = (b) => {
      const profit = (+b.selling_price || 0) - (+b.total_cost || 0);
      const n = daysUntil(b.arrival_date);
      const cd = n == null ? "" : n === 0 ? "msa-text-green" : n > 0 && n <= 14 ? "msa-text-orange" : n < 0 ? "msa-dim" : "msa-text-brand";
      const ex = !!expanded[b.id];
      const act = bookingActivity(b);
      const _T = localDayStr(/* @__PURE__ */ new Date());
      const _a = dayOf(b.arrival_date), _dep = dayOf(b.departure_date);
      ...
      const main = h(
        "tr",
        { key: b.id, className: "msa-bt-row" + (ex ? " open" : "") + (tone ? " " + tone : "") + (act ? " msa-bt-glow" : ""), onClick: () => setExpanded((s) => ({ ...s, [b.id]: !s[b.id] })) },
        ...
        h(
          "td",
          { "data-label": "", className: "msa-right msa-actions" },
          h("button", { className: "msa-icon-btn", title: "Edit", onClick: (e) => {
            e.stopPropagation();
            setEdit(b);
          } }, ICON.edit()),
          h("button", { className: "msa-icon-btn", title: "Itinerary PDF", onClick: (e) => {
            e.stopPropagation();
            setDoc({ booking: b, type: "itinerary" });
          } }, ICON.doc()),
          isAdminRole() && h("button", { className: "msa-icon-btn msa-ic-green", title: "Invoice PDF", onClick: (e) => {
            e.stopPropagation();
            setDoc({ booking: b, type: "invoice" });
          } }, ICON.invoice()),
      ...
      const detail = h("tr", { key: b.id + "-d", className: "msa-bt-detail" }, h(
        "td",
        { colSpan: 10 },
        h(
          "div",
          { className: "msa-bt-detail-grid" },
          h("div", null, h("span", { className: "msa-fin-k" }, "Route & dates"), h("div", null, (b.arrival_city || "") + " \u2192 " + (b.departure_city || "")), h("div", { className: "msa-dim" }, (b.total_nights || 0) + " nights / " + (b.total_days || 0) + " days \xB7 " + countdownLabel(b.arrival_date))),
        ...
        h(
          "div",
          { className: "msa-bt-detail-actions" },
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Bookings"), h("p", { className: "msa-subtitle" }, "Trips & inquiries management")),
        h("button", { className: "msa-btn msa-btn-primary", onClick: () => setEdit(EMPTY_BOOKING) }, ICON.plus(), "New Booking")
      ),
      h(
        "div",
        { className: "msa-searchbar" },
      ...
      h(
        "div",
        { className: "msa-table-card" },
        active.length === 0 ? h("div", { className: "msa-empty" }, "No active bookings.") : table(active),
      ...
      edit && h(BookingModal, { initial: edit.id ? edit : EMPTY_BOOKING, onClose: () => setEdit(null), onSaved: () => {
        setEdit(null);
        reload();
      }, onView: (bk, t) => setDoc({ booking: bk, type: t }), suppliers }),
      doc && h(DocModal, { booking: doc.booking, initialType: doc.type, settings, onClose: () => setDoc(null), onEdit: (bk) => {
        setDoc(null);
        setEdit(bk);
      } })
    );
  ...
  function Clients({ clients, bookings, reload, initialQuery }) {
    ...
    const [sort, setSort] = useState({ k: "name", d: "asc" });
    const [expanded, setExpanded] = useState({});
    useEffect(() => {
    ...
    const add = async () => {
      ...
      await dbInsert("clients", { ...f, email: f.email ? f.email.toLowerCase() : null, trips: 0, created_by: CURRENT_EMAIL || DEFAULT_ADMIN_EMAIL });
      setF({ name: "", email: "", phone: "", country: "" });
      setAdding(false);
      reload();
    };
    const del = async (c, e) => {
    ...
    const toggleSort = (k) => setSort((s) => s.k === k ? { k, d: s.d === "asc" ? "desc" : "asc" } : { k, d: "asc" });
    const exportCSV = () => {
      const head = ["Name", "Email", "Phone", "Country", "Trips", "Total Spent", "Profit", "Last Trip"];
      const esc = (s) => '"' + String(s == null ? "" : s).replace(/"/g, '""') + '"';
      const lines = [head.join(",")];
      list.forEach((c) => lines.push([c.name, c.email, c.phone, c.country, tripsFor(c), c.total_spent, profitFor(c), c.last_trip].map(esc).join(",")));
      const blob = new Blob([lines.join("\n")], { type: "text/csv" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "marrakechstory-clients.csv";
      a.click();
      URL.revokeObjectURL(url);
    };
    ...
    const row = (c) => {
      const profit = profitFor(c);
      const trips = tripsFor(c);
      const ex = !!expanded[c.id];
      const cb = bookingsFor(c);
      const main = h(
        "tr",
        { key: c.id, className: "msa-bt-row" + (ex ? " open" : ""), onClick: () => setExpanded((s) => ({ ...s, [c.id]: !s[c.id] })) },
      ...
      if (!ex) return main;
      const detail = h("tr", { key: c.id + "-d", className: "msa-bt-detail" }, h(
        "td",
        { colSpan: 9 },
        h(
          "div",
        ...
        cb.length > 0 && h("div", { className: "msa-cl-trips" }, cb.map((b) => {
          ...
          return h(
            "div",
            { key: b.id, className: "msa-cl-bk" },
            h(
              "div",
              { className: "msa-cl-bk-head", onClick: () => setExpanded((s) => ({ ...s, ["itin-" + b.id]: !s["itin-" + b.id] })) },
            ...
            itin.length > 0 && expanded["itin-" + b.id] ? h("div", { className: "msa-cl-itin" }, itin.map((d, i) => h(
              "div",
              { key: i, className: "msa-cl-itin-day" },
              h("div", { className: "msa-cl-itin-n" }, d.day || i + 1),
              h(
                "div",
                { className: "msa-cl-itin-body" },
                h("strong", null, (d.city || "Day " + (d.day || i + 1)) + (d.date ? " \xB7 " + fmtDate(d.date) : "")),
                (d.activities || []).map((a, ai) => h("div", { key: ai, className: "msa-cl-itin-act" }, (a.time ? a.time + " \xB7 " : "") + (a.type || "") + (a.details ? " \u2014 " + a.details : "")))
              )
            ))) : null,
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Clients"), h("p", { className: "msa-subtitle" }, clients.length + " clients \xB7 " + kr(clients.reduce((s, c) => s + (+c.total_spent || 0), 0)) + " lifetime value")),
      ...
      adding && h(
        "div",
        { className: "msa-card msa-inline-form" },
        h(
          "div",
          { className: "msa-grid-4" },
      ...
      h("div", { className: "msa-table-card" }, list.length === 0 ? h("div", { className: "msa-empty" }, "No clients found.") : h("table", { className: "msa-table msa-btable" }, h("thead", null, h("tr", null, h("th", { className: "msa-bt-chev" }, ""), COLS.map(([k, l]) => headCell(k, l)), h("th", null, ""))), h("tbody", null, list.map(row))))
    );
  }
  function CollaboratorModal({ initial, onClose, onSaved }) {
    ...
    const upload = async (fl) => {
      const sb = getSB();
      if (!sb || !s.id) {
        alert("Save the collaborator first, then upload files.");
        return;
      }
      ...
      for (const f of arr) {
        ...
        const { error } = await sb.storage.from(BUCKET).upload("collab/" + s.id + "/" + Date.now() + "-" + safe, f, { cacheControl: "3600", contentType: f.type || void 0 });
        if (error) alert("Upload failed (" + f.name + "): " + error.message);
      }
      setUpBusy(false);
      loadDocs();
    };
    const delDoc = async (name) => {
      ...
      if (!sb || !s.id) return;
      if (!confirm("Delete this file?")) return;
      await sb.storage.from(BUCKET).remove(["collab/" + s.id + "/" + name]);
      loadDocs();
    };
    ...
    return h("div", { className: "msa-modal-backdrop", onClick: onClose }, h(
      ...
      h(
        ...
        h(
          ...
          fld("City", "city"),
          fld("Contact person", "contact"),
          fld("Phone", "phone"),
          fld("Second phone", "phone2", "text", "optional"),
          fld("Email", "email", "email"),
          fld("Rate", "rate", "text", "e.g. 1200 DH/night"),
        ...
        h(
          "h4",
          { className: "msa-section msa-section-row" },
          h("span", null, "Documents \u2014 price lists, menus, contracts"),
          s.id ? h("label", { className: "msa-btn msa-btn-sm msa-btn-primary msa-upload-label" }, ICON.plus(), upBusy ? "Uploading\u2026" : "Upload", h("input", { type: "file", multiple: true, disabled: upBusy, style: { display: "none" }, onChange: (e) => {
            upload(e.target.files);
            e.target.value = "";
          } })) : h("span", { className: "msa-dim", style: { fontSize: 12, fontWeight: 400 } }, "Save first to attach files")
        ),
        s.id ? h("div", { className: "msa-files" }, docs.length === 0 ? h("div", { className: "msa-dim", style: { padding: "6px 2px" } }, "No documents yet \u2014 upload price lists, menus or contracts.") : docs.map((f) => h(
          "div",
          { key: f.name, className: "msa-file-row" },
          h("a", { className: "msa-file-name", href: "#", onClick: (e) => {
            e.preventDefault();
          ...
          h("button", { className: "msa-icon-btn", title: "Delete", onClick: () => delDoc(f.name) }, ICON.trash())
        ))) : null
      )
    ));
  }
  function Suppliers({ suppliers, bookings, leads, reload, seed, clearSeed }) {
    ...
    const spendFor = (s) => {
      ...
      const hist = [];
      (bookings || []).forEach((b) => {
        const amt = bookingCollabSpend(b).filter((l) => l.collab === s.id).reduce((t, l) => t + l.amount, 0);
        if (amt > 0) {
          total += amt;
          hist.push({ b, amt });
        }
    ...
    const addFromLead = (l) => {
      const p = l.payload || {};
      setEdit({ type: "hotel", name: l.name || "", city: l.country || "", contact: l.name || "", phone: l.phone || "", email: l.email || "", notes: (p.collaborationType ? "Type: " + p.collaborationType + ". " : "") + (p.message || "") });
    };
    ...
    const list = suppliers.filter((s) => !q || [s.name, s.city, s.phone, s.phone2, s.email, s.type, typeLabel(s.type)].join(" ").toLowerCase().includes(q.toLowerCase())).sort((a, b) => {
      const p = sorter(a, b);
      if (p !== 0) return p;
    ...
    const row = (s) => {
      const sp = isAdminRole() ? spendFor(s) : { total: 0, hist: [] };
      const ex = !!expanded[s.id];
      const main = h(
        "tr",
        { key: s.id, className: "msa-bt-row" + (ex ? " open" : ""), onClick: () => setExpanded((p) => ({ ...p, [s.id]: !p[s.id] })) },
        h("td", { className: "msa-bt-chev", "data-label": "" }, h("span", { className: "msa-chevtog" }, ex ? "\u2303" : "\u2304")),
      ...
      if (!ex) return main;
      const detail = h("tr", { key: s.id + "-d", className: "msa-bt-detail" }, h(
        "td",
        { colSpan: 8 },
        h(
          "div",
          ...
          h(
            "div",
            null,
            h("span", { className: "msa-fin-k" }, "Contact"),
        ...
        isAdminRole() && sp.hist.length ? h("div", { className: "msa-cl-trips" }, sp.hist.map(({ b, amt }) => h(
          "div",
          { key: b.id, className: "msa-cl-bk" },
          h(
            "div",
            { className: "msa-cl-bk-head" },
            h("span", { className: "msa-ref-chip" }, b.reference || "\u2014"),
            h("span", { className: "msa-cl-bk-route" }, b.client_name || ""),
            h("span", { className: "msa-badge msa-st-" + b.status }, STATUS_LABEL[b.status]),
            h("span", { className: "msa-text-brand", style: { fontWeight: 700 } }, kr(amt))
          ),
          h("div", { className: "msa-cl-bk-sub" }, fmtDate(b.arrival_date) + " \u2192 " + fmtDate(b.departure_date))
        ))) : null
      ));
      return [main, detail];
    };
    const spendOverview = function() {
      ...
      const catTotals = {};
      SUP_CATS.forEach(([c]) => catTotals[c] = 0);
      perSup.forEach(({ s, total }) => {
        const c = SUP_CAT[s.type] || "activities";
        catTotals[c] += total;
      });
      ...
      return h(
        "div",
        { className: "msa-card msa-sup-spend" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Spend overview"), h("span", { className: "msa-dim" }, "Total " + kr(grand))),
        h(
          "div",
          { className: "msa-sup-spend-grid" },
          h(
            "div",
            null,
            h("h4", { className: "msa-sub" }, "By category"),
            h(
              "div",
              { className: "msa-chart-row" },
              h(Donut, { segments }),
              h("div", { className: "msa-legend" }, segments.map((s, i) => h(
                "div",
                { key: i },
                h("span", { className: "msa-dot", style: { background: s.color } }),
                s.label + " ",
                h("strong", null, kr(s.value)),
                h("span", { className: "msa-dim", style: { marginLeft: 4 } }, "\xB7 " + Math.round(s.value / grand * 100) + "%")
              )))
            )
          ),
          h(
            "div",
            null,
            h("h4", { className: "msa-sub" }, "Top collaborators"),
            h("div", { className: "msa-srcbars" }, top.map(({ s, total }, i) => h(
              "div",
              { key: i, className: "msa-srcbar" },
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Collaborators"), h("p", { className: "msa-subtitle" }, suppliers.length + " partners \xB7 " + pending.length + " pending requests")),
        h("button", { className: "msa-btn msa-btn-primary", onClick: () => setEdit({ type: "hotel" }) }, ICON.plus(), "Add")
      ),
      pending.length > 0 && h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Pending collaboration requests")),
        h("div", { className: "msa-pending-list" }, pending.map((l) => h(
          "div",
          { key: l.id, className: "msa-pending" },
          h("div", null, h("strong", null, l.name || l.email || "Anonymous"), h("div", { className: "msa-dim" }, [l.payload && l.payload.collaborationType, l.email, l.phone].filter(Boolean).join(" \xB7 "))),
      ...
      list.length === 0 ? h("div", { className: "msa-table-card" }, h("div", { className: "msa-empty" }, "No collaborators yet. Click \u201CAdd\u201D to create one.")) : SUP_CATS.map(([cat, label]) => {
        const rows = list.filter((s) => (SUP_CAT[s.type] || "activities") === cat);
        if (!rows.length) return null;
        return h(
          "div",
          { key: cat, className: "msa-supcat" },
  ...
  function Finance({ bookings, suppliers }) {
    const collabSpend = function() {
      ...
      return Object.keys(m).map((id) => {
        ...
        return { id, name: s ? s.name : "Unknown", type: s ? s.type : "", total: m[id].total, count: m[id].count };
    ...
    bookings.forEach((b) => {
      ...
      const k = b.arrival_date.slice(0, 7);
      byMonth[k] = (byMonth[k] || 0) + (+b.selling_price || 0);
    });
    const months = Object.keys(byMonth).sort().slice(-8).map((k) => ({ label: k.slice(5) + "/" + k.slice(2, 4), value: byMonth[k] }));
    const bySource = {};
    bookings.forEach((b) => {
      const k = b.lead_source || "other";
    ...
    const byYearP = {};
    datedB.forEach((b) => {
      const y = b.arrival_date.slice(0, 4);
      byYearP[y] = (byYearP[y] || 0) + profitOf(b);
    });
    ...
    datedB.forEach((b) => {
      const k = b.arrival_date.slice(0, 7);
      byMonthP[k] = (byMonthP[k] || 0) + profitOf(b);
    });
    ...
    return h(
      "div",
      { className: "msa-page" },
      h("header", { className: "msa-page-head" }, h("h1", null, "Finance"), h("p", null, "Revenue, costs and profitability \xB7 " + bookings.length + " bookings")),
      ...
      h(
        "div",
        { className: "msa-card msa-fin-breakdown" },
        h(
          "div",
          { className: "msa-card-head" },
          h("h3", null, "Benefit breakdown"),
          firstD ? h("span", { className: "msa-dim" }, "From " + fmtDate(firstD) + (lastD && lastD !== firstD ? " to " + fmtDate(lastD) : "") + " \xB7 " + spanDays + " days") : null
        ),
        firstD ? h(
          R.Fragment,
          null,
          ...
          h(
            "div",
            { className: "msa-cols msa-cols-12", style: { marginTop: 16 } },
            h(
              "div",
              null,
              h("div", { className: "msa-fin-k", style: { marginBottom: 8 } }, "Net profit by year"),
              h("div", { className: "msa-srcbars" }, yearsP.map((yy, i) => {
                const v = byYearP[yy];
                return h(
                  "div",
                  { key: i, className: "msa-srcbar" },
                  h("span", { className: "msa-srcbar-l" }, yy),
                  h("div", { className: "msa-srcbar-track" }, h("div", { className: "msa-srcbar-fill", style: { width: Math.abs(v) / maxYearP * 100 + "%", background: v >= 0 ? "#34c759" : "#e0432a" } })),
            ...
            h(
              "div",
              null,
              h("div", { className: "msa-fin-k", style: { marginBottom: 8 } }, "Net profit by month"),
              h("div", { className: "msa-srcbars" }, monthsP.map((k, i) => {
                const v = byMonthP[k];
                return h(
                  "div",
                  { key: i, className: "msa-srcbar" },
                  h("span", { className: "msa-srcbar-l" }, monLbl(k)),
                  h("div", { className: "msa-srcbar-track" }, h("div", { className: "msa-srcbar-fill", style: { width: Math.abs(v) / maxMonthP * 100 + "%", background: v >= 0 ? "#34c759" : "#e0432a" } })),
                  h("span", { className: "msa-srcbar-v" }, kr(v))
                );
              }))
            )
      ...
      h(
        "div",
        { className: "msa-cols msa-cols-12" },
        h(
          "div",
          { className: "msa-card" },
          h("div", { className: "msa-card-head" }, h("h3", null, "Cost breakdown")),
          h(
            "div",
            { className: "msa-chart-row" },
            h(Donut, { segments: [{ label: "Accommodation", value: acc, color: "#e0432a" }, { label: "Transportation", value: tr, color: "#0a84ff" }, { label: "Activities", value: ac, color: "#34c759" }] }),
            h(
              "div",
              { className: "msa-legend" },
      ...
      h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Lead sources")),
      ...
      h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Spend by collaborator"), h("span", { className: "msa-dim" }, collabSpend.length + " partners")),
        collabSpend.length === 0 ? h("div", { className: "msa-empty" }, "Assign collaborators to booking cost lines (Transport / Accommodation / Activities) to track spend here.") : h("div", { className: "msa-srcbars" }, collabSpend.map((c, i) => h(
          "div",
          { key: i, className: "msa-srcbar" },
          h("span", { className: "msa-srcbar-l", title: c.name }, c.name),
          h("div", { className: "msa-srcbar-track" }, h("div", { className: "msa-srcbar-fill", style: { width: c.total / maxCollab * 100 + "%" } })),
          h("span", { className: "msa-srcbar-v" }, kr(c.total) + " \xB7 " + c.count)
        )))
      ),
      h(
        "div",
        { className: "msa-card msa-card-flush" },
        h("div", { className: "msa-card-head msa-pad" }, h("h3", null, "Breakdown by booking")),
        h(
          "table",
          { className: "msa-table" },
          h("thead", null, h("tr", null, ["Reference", "Client", "Price", "Cost", "Profit", "Margin", "Status"].map((c, i) => h("th", { key: i, className: i >= 2 && i <= 5 ? "msa-right" : "" }, c)))),
          h("tbody", null, bookings.slice().sort((a, b) => (+b.selling_price || 0) - (+a.selling_price || 0)).map((b) => {
            const s = +b.selling_price || 0, c = +b.total_cost || 0, p = s - c;
            return h(
              "tr",
              { key: b.id },
              h("td", { "data-label": "Reference" }, h("span", { className: "msa-ref-chip" }, b.reference || "\u2014")),
              h("td", { "data-label": "Client" }, b.client_name),
              h("td", { "data-label": "Price", className: "msa-right" }, kr(s)),
              h("td", { "data-label": "Cost", className: "msa-right msa-text-red" }, kr(c)),
              h("td", { "data-label": "Profit", className: "msa-right msa-text-green" }, kr(p)),
              h("td", { "data-label": "Margin", className: "msa-right" }, s ? Math.round(p / s * 100) + "%" : "\u2014"),
              h("td", { "data-label": "Status" }, h("span", { className: "msa-badge msa-st-" + b.status }, STATUS_LABEL[b.status]))
            );
          }))
        )
  ...
  function TaskModal({ initial, onClose, onSaved }) {
    ...
    const save = async () => {
      ...
      if (t.status === "completed" && (!initial || initial.status !== "completed")) {
        row.done_by = CURRENT_EMAIL;
        row.done_at = (/* @__PURE__ */ new Date()).toISOString();
      }
      if (initial && initial.id) {
        await dbUpdate("tasks", initial.id, row);
        logAudit("updated task", "workspace", initial.id, row.title + " \u2192 " + assignLabel(row.assigned_to));
      } else {
        await dbInsert("tasks", { ...row, created_by: CURRENT_EMAIL });
        logAudit("added task", "workspace", null, row.title + " \u2192 " + assignLabel(row.assigned_to));
      }
      onSaved();
    };
    return h("div", { className: "msa-modal-backdrop", onClick: onClose }, h(
  ...
  function Workspace({ tasks, reload }) {
    ...
    const [quickTo, setQuickTo] = useState(role === "admin" ? "partner" : "admin");
    ...
    const del = async (t) => {
      if (confirm("Delete this task?")) {
        ...
        logAudit("deleted task", "workspace", t.id, t.title);
        reload();
      }
    };
    ...
    const visible = seg === "done" ? all.filter((t) => t.status === "completed") : seg === "mine" ? open.filter(mineFor) : open;
    const whoName = (email) => email === CURRENT_EMAIL ? "you" : normEmail(email) === normEmail(DEFAULT_ADMIN_EMAIL) ? "Admin" : "Partner";
    const card = (t) => {
      const n = daysUntil((t.due || "").slice(0, 10));
      const overdue = t.status !== "completed" && n != null && n < 0;
      const soon = t.status !== "completed" && n != null && n >= 0 && n <= 2;
      return h(
        "div",
        { key: t.id, className: "msa-ws-task" + (t.status === "completed" ? " done" : "") + (overdue ? " overdue" : soon ? " soon" : "") },
        h("button", { className: "msa-check", onClick: () => toggle(t) }, t.status === "completed" ? "\u2713" : ""),
        h(
          "div",
          { className: "msa-ws-body", onClick: () => setEdit(t) },
          h(
            "div",
            { className: "msa-ws-line1" },
          ...
          h(
            "div",
            { className: "msa-ws-meta" },
            h("span", { className: overdue ? "msa-text-red" : soon ? "msa-text-orange" : "msa-dim" }, (t.due ? fmtDate(t.due) : "No due date") + (overdue ? " \xB7 Overdue" : soon ? " \xB7 Due soon" : "")),
            t.created_by ? h("span", { className: "msa-dim" }, " \xB7 added by " + whoName(t.created_by)) : null,
            t.status === "completed" && t.done_by ? h("span", { className: "msa-dim" }, " \xB7 done by " + whoName(t.done_by)) : null
          )
        ),
    ...
    return h(
      "div",
      { className: "msa-page msa-narrow" },
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Workspace"), h("p", { className: "msa-subtitle" }, "Shared to-dos for the team \xB7 " + open.length + " open")),
        h("button", { className: "msa-btn msa-btn-primary", onClick: () => setEdit({ assigned_to: role === "admin" ? "partner" : "admin" }) }, ICON.plus(), "New task")
      ),
      h(
        "div",
        { className: "msa-ws-quick" },
        h("input", { className: "msa-ws-quick-in", value: quick, placeholder: "Quick add a task\u2026", onChange: (e) => setQuick(e.target.value), onKeyDown: (e) => {
      ...
      h(
        "div",
        { className: "msa-toolbar", style: { gap: 10 } },
      ...
      h("div", { className: "msa-card" }, visible.length === 0 ? h("div", { className: "msa-empty" }, seg === "done" ? "Nothing completed yet." : "All caught up! \u{1F389}") : visible.map(card)),
      edit && h(TaskModal, { initial: edit, onClose: () => setEdit(null), onSaved: () => {
        setEdit(null);
        reload();
      } })
    );
  ...
  function CalendarTab({ bookings, openBooking }) {
    ...
    const dayMap = useMemo(() => {
      const m = {};
      bookings.forEach((b) => {
        if (b.arrival_date) (m[b.arrival_date] = m[b.arrival_date] || { arr: [], dep: [], on: [] }).arr.push(b);
        if (b.departure_date) (m[b.departure_date] = m[b.departure_date] || { arr: [], dep: [], on: [] }).dep.push(b);
        if (b.arrival_date && b.departure_date) {
          let d = new Date(b.arrival_date);
          const e = new Date(b.departure_date);
          let g = 0;
    ...
    const prev = () => {
      if (view === "year") setCursor(new Date(Y - 1, M, 1));
      else if (view === "month") setCursor(new Date(Y, M - 1, 1));
      else {
        const d = new Date(sel);
        d.setDate(d.getDate() - 1);
        setSel(d.toISOString().slice(0, 10));
      }
    };
    const next = () => {
      if (view === "year") setCursor(new Date(Y + 1, M, 1));
    ...
    const miniMonth = (mi) => {
      const first = mondayOffset(new Date(Y, mi, 1));
      ...
      return h("div", { key: mi, className: "msa-yr-month" }, h("div", { className: "msa-yr-name", onClick: () => {
        ...
        setView("month");
      } }, MON[mi]), h("div", { className: "msa-yr-days" }, cells));
    };
    const todayLong = (/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
    ...
    const yearView = () => h(
      ...
      h(
        "div",
        { className: "msa-card", style: { marginTop: 16 } },
        h(
          "div",
          { className: "msa-card-head" },
          h("h3", null, "Today \xB7 " + todayLong + (todayCount ? "  \xB7  " + todayCount + " booking" + (todayCount === 1 ? "" : "s") : "")),
    ...
    const monthGrid = () => {
      ...
      for (let d = 1; d <= dim; d++) {
        ...
        const bars = info.on.slice(0, 4).map((b) => {
          ...
          return h(
            "div",
            { key: b.id, className: "msa-cal-ev" + (isStart ? " start" : "") + (isEnd ? " end" : "") + (!isStart && !isEnd ? " mid" : ""), style: { background: c }, title: b.client_name + (isStart ? " \u2014 arrival" : isEnd ? " \u2014 departure" : ""), onClick: (e) => {
              e.stopPropagation();
              openBooking(b);
            } },
    ...
    const dayPanel = (k, showOpen) => {
      ...
      const byId = {};
      info.arr.forEach((b) => {
        (byId[b.id] = byId[b.id] || { b, kinds: [] }).kinds.push("Arrival");
      });
      info.dep.forEach((b) => {
        (byId[b.id] = byId[b.id] || { b, kinds: [] }).kinds.push("Departure");
      });
      info.on.forEach((b) => {
        if (!byId[b.id]) byId[b.id] = { b, kinds: ["On trip"] };
      });
      ...
      return h("div", { className: "msa-cal-progs" }, list.map(({ b, kinds }) => {
        ...
        return h(
          ...
          h(
            ...
            b.phone ? h("a", { href: "tel:" + b.phone, className: "msa-cal-prog-link" }, "\u{1F4DE} " + b.phone) : null,
            prog && prog.city ? h("span", { className: "msa-cal-prog-city" }, "\u{1F4CD} " + prog.city) : null
          ),
          h(
            "div",
            { className: "msa-cal-acts" },
            ...
            acts.length ? acts.map((a, i) => h("div", { key: i, className: "msa-cal-act" }, h("span", { className: "msa-cal-act-t" }, a.time || "\xB7"), h("div", null, h("strong", null, a.type || ""), a.details ? h("p", null, a.details) : null))) : prog ? null : h("div", { className: "msa-dim", style: { fontSize: 12.5, padding: "4px 2px" } }, "No detailed program set for this day yet."),
          ...
          b.special_requests ? h("div", { className: "msa-cal-prog-note" }, "\u{1F4DD} " + b.special_requests) : null
        );
      }));
    };
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Calendar"), h("p", { className: "msa-subtitle" }, "Today is " + (/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" }))),
  ...
  function Requests({ leads, bookings, reload, settings, suppliers }) {
    ...
    const aiSeen = React.useRef(/* @__PURE__ */ new Set());
    const setNote = (id, v) => setNotes((s) => ({ ...s, [id]: v }));
    const setReply = (id, v) => setReplies((s) => ({ ...s, [id]: v }));
    const flashMsg = (id, m) => {
      setFlash((s) => ({ ...s, [id]: m }));
      setTimeout(() => setFlash((s) => ({ ...s, [id]: "" })), 4e3);
    };
    const DKEY = (id) => "msa_req_draft_" + id;
    const setDraft = (id, v) => {
      setReply(id, v);
      setDraftFlag((s) => ({ ...s, [id]: true }));
      try {
        localStorage.setItem(DKEY(id), v);
      } catch (e) {
      }
    };
    const aiUpgrade = async (l, force) => {
      ...
      if (txt) {
        setDraft(l.id, txt);
        if (force) flashMsg(l.id, "AI draft ready" + (provider ? " (" + provider + ")" : "") + " \u2713");
      } else if (force) {
    ...
    const deleteDraft = (l) => {
      setReply(l.id, "");
      setDraftFlag((s) => ({ ...s, [l.id]: false }));
      try {
        localStorage.setItem(DKEY(l.id), "");
      } catch (e) {
      }
    };
    const clearDraft = (id) => {
      setDraftFlag((s) => ({ ...s, [id]: false }));
      try {
        localStorage.setItem(DKEY(id), "");
      } catch (e) {
      }
    };
    React.useEffect(() => {
      Object.keys(expanded).forEach((id) => {
        ...
        if (cur && cur !== agentDraft(l)) return;
        aiUpgrade(l, false);
      });
    }, [expanded]);
    React.useEffect(() => {
      ...
      (leads || []).forEach((l) => {
        let s = null;
        try {
          s = localStorage.getItem(DKEY(l.id));
        } catch (e) {
        }
        if (s != null) {
          if (s) {
          ...
          const txt = agentDraft(l);
          initR[l.id] = txt;
          initF[l.id] = true;
          try {
            localStorage.setItem(DKEY(l.id), txt);
          } catch (e) {
          }
        }
      });
    ...
    const replyProfile = async (l) => {
      ...
      if (!body) return;
      if (!l.email) {
        flashMsg(l.id, "No client email \u2014 use WhatsApp instead");
        return;
      }
      const sb = getSB();
      const b = bkFor(l);
      ...
      if (error) {
        flashMsg(l.id, "Failed: " + error.message);
        return;
      }
      ...
      setReply(l.id, "");
      clearDraft(l.id);
      flashMsg(l.id, "Sent to client profile \u2713");
      reload();
    };
    const replyEmail = (l) => {
      const body = (replies[l.id] || "").trim();
    ...
    const replyWhatsapp = (l) => {
      const body = (replies[l.id] || "").trim();
      if (l.phone) {
        window.open(waLink(l.phone) + (body ? "?text=" + encodeURIComponent(body) : ""), "_blank");
        logAudit("reply", "request", l.id, "via WhatsApp");
    ...
    const docBooking = (l, b) => b || {
      ...
      arrival_city: l.payload && l.payload.arriveCity || "Marrakech",
      departure_city: l.payload && l.payload.departCity || "Marrakech",
      arrival_date: l.start_date,
      departure_date: l.end_date,
      total_days: l.duration || 0,
      total_nights: Math.max(0, (l.duration || 1) - 1),
    ...
    const del = async (l, e) => {
      ...
      if (confirm("Delete this request?")) {
        await dbDelete("form_submissions", l.id);
        reload();
      }
    };
    const markHandled = async (l, e) => {
      e && e.stopPropagation();
      await dbUpdate("form_submissions", l.id, { handled: !l.handled });
      logAudit(l.handled ? "reopened" : "handled", "request", l.id, l.name || l.email || "");
      reload();
    };
    const list = leads.filter((l) => !q || [l.name, l.email, l.phone, l.kind, l.trip_type, l.country, l.payload && l.payload.bookingCtx && l.payload.bookingCtx.title].join(" ").toLowerCase().includes(q.toLowerCase())).sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
    const row = (l) => {
      const b = bkFor(l);
      const p = l.payload || {};
      ...
      const main = h(
        "tr",
        { key: l.id, className: "msa-bt-row" + (ex ? " open" : "") + (!l.handled ? " msa-rq-new" : ""), onClick: () => setExpanded((s) => ({ ...s, [l.id]: !s[l.id] })) },
        h("td", { className: "msa-bt-chev", "data-label": "" }, h("span", { className: "msa-chevtog" }, ex ? "\u2303" : "\u2304")),
        ...
        h(
          "td",
          { "data-label": "Booked" },
          title === "\u2014" ? h("span", { className: "msa-dim" }, [l.trip_type, l.duration ? l.duration + "d" : null].filter(Boolean).join(" \xB7 ") || "\u2014") : h("strong", { className: "msa-text-brand" }, title),
          h("div", { className: "msa-dim", style: { fontSize: 11, marginTop: 2 } }, [catLabel || reqKindLabel(l), p.people ? p.people + " pax" : null].filter(Boolean).join(" \xB7 "))
        ),
        ...
        h(
          "td",
          { "data-label": "", className: "msa-right msa-actions" },
          b && h("button", { className: "msa-icon-btn", title: "Edit booking", onClick: (e) => {
            e.stopPropagation();
            setEditBk(b);
          } }, ICON.edit()),
          (b || itin.length) && h("button", { className: "msa-icon-btn", title: "Itinerary PDF", onClick: (e) => {
            e.stopPropagation();
            setDoc({ booking: docBooking(l, b), type: "itinerary" });
          } }, ICON.doc()),
          b && isAdminRole() && h("button", { className: "msa-icon-btn msa-ic-green", title: "Invoice PDF", onClick: (e) => {
            e.stopPropagation();
            setDoc({ booking: b, type: "invoice" });
          } }, ICON.invoice()),
      ...
      const flights = [p.arriveCity ? "In: " + p.arriveCity : "", p.departCity ? "Out: " + p.departCity : "", p.flightBooked ? "Booked: " + p.flightBooked : ""].filter(Boolean).join("  \xB7  ");
      ...
      const transport = p.needTransport ? "Transport requested" + (p.pickupAddr ? " \xB7 pickup: " + reqText(p.pickupAddr) : "") : "";
      ...
      }).filter(Boolean).sort((a, b2) => reqOrderIdx(a.key) - reqOrderIdx(b2.key));
      const detail = h("tr", { key: l.id + "-d", className: "msa-bt-detail" }, h(
        "td",
        { colSpan: 8 },
        p.chooseForMe ? h("div", { className: "msa-rq-flag" }, "\u2B50 Choose for me \u2014 client wants MarrakechStory to craft the trip") : null,
        h(
          "div",
          { className: "msa-rq-head" },
          h(
            "div",
            { className: "msa-rq-head-contact" },
            l.email && h("a", { href: "mailto:" + l.email }, l.email),
            l.phone && h("a", { href: waLink(l.phone), target: "_blank" }, l.phone),
            h("span", { className: "msa-dim" }, [l.country, "via " + (l.via || l.kind)].filter(Boolean).join(" \xB7 "))
          ),
          h("div", { className: "msa-rq-head-trip" }, [title && title !== "\u2014" ? title : null, catLabel || reqKindLabel(l), p.people || pax ? (p.people || pax) + " pax" : null, l.duration ? l.duration + " days" : null, dateStr || null, flights || null, hasStyle ? [p.accommodation, p.pace, p.budget].filter(Boolean).join(" / ") : null].filter(Boolean).join("  \xB7  "))
        ),
        p.notes || p.avoid ? h("div", { className: "msa-bk-notes", style: { marginTop: 10 } }, [p.notes && reqText(p.notes), p.avoid && "Avoid: " + reqText(p.avoid)].filter(Boolean).join(" \xB7 ")) : null,
        dump.length ? h(
          "details",
          { className: "msa-rq-dump" },
          ...
          h("ul", { className: "msa-rq-simplelist" }, dump.map((d, i) => h(
            "li",
            { key: i, className: "msa-rq-sl-row" },
            h("span", { className: "msa-rq-sl-k" }, d.label),
            h("span", { className: "msa-rq-sl-v" }, d.value)
          )))
        ) : null,
        itin.length > 0 ? h("div", { className: "msa-rq-itin", style: { marginTop: 12 } }, itin.map((d, i) => h(
          "div",
          { key: i, className: "msa-rq-day" },
          h("div", { className: "msa-rq-day-n" }, d.day || i + 1),
          h(
            "div",
            null,
            h("strong", null, (d.city || "Day " + (d.day || i + 1)) + (d.date ? " \xB7 " + fmtDate(d.date) : "")),
            (d.activities || []).map((a, ai) => h("div", { key: ai, className: "msa-dim" }, (a.time ? a.time + " \xB7 " : "") + (a.type || "") + (a.details ? " \u2014 " + a.details : "")))
          )
        ))) : null,
        ...
        h(
          ...
          h(
            "div",
            { className: "msa-agent-top" },
            h(
              "div",
              { className: "msa-agent-id" },
              h("span", { className: "msa-agent-av" + (agentBusy[l.id] ? " msa-agent-av-busy" : "") }, "\u2726"),
              h("span", { className: "msa-agent-name" }, "MarrakechStory AI agent"),
              agentBusy[l.id] ? h("span", { className: "msa-agent-badge" }, "Writing\u2026") : (replies[l.id] || "").trim() ? h("span", { className: "msa-agent-badge" + (draftFlag[l.id] ? "" : " msa-agent-badge-edit") }, draftFlag[l.id] ? "Draft \u2014 review before sending" : "Edited") : h("span", { className: "msa-dim", style: { fontSize: 12 } }, "No draft")
            ),
          ...
          h(
            "div",
            { className: "msa-agent-btns" },
            h(
              "div",
              { className: "msa-rq-act-grp" },
        ...
        h(
          ...
          h(
            "div",
            { className: "msa-rq-act-grp msa-rq-act-end" },
    ...
    return h(
      ...
      h("div", { className: "msa-table-card" }, list.length === 0 ? h("div", { className: "msa-empty" }, "No requests yet.") : h("table", { className: "msa-table msa-btable" }, h("thead", null, h("tr", null, h("th", { className: "msa-bt-chev" }, ""), ["Date", "Client", "Itinerary", "Dates", "Status"].map((c, i) => h("th", { key: i }, c)), h("th", null, ""))), h("tbody", null, list.map(row)))),
      doc && h(DocModal, { booking: doc.booking, initialType: doc.type, settings, onClose: () => setDoc(null) }),
      editBk && h(BookingModal, { initial: editBk, onClose: () => setEditBk(null), onSaved: () => {
        setEditBk(null);
        reload();
      }, onView: (bk, t) => setDoc({ booking: bk, type: t }), suppliers })
    );
  ...
  const SM_COMPANIES = [
    { key: "colombani", name: "COLOMBANI D\xB4EXPLOITATION DES RIAD", ice: "001458075000007" },
    { key: "stegor", name: "STREGOR SARL", ice: "000071621000025" }
  ];
  const SM_INVOICE = {
    services: [
      "Cr\xE9ation du r\xE9tro-planning des publications / Stories SM",
      "Cr\xE9ation des accroches selon la ligne \xE9ditoriale",
      "R\xE9alisation des r\xE9els convenus",
      "Publication des posts selon le planning confirm\xE9"
    ],
    price: "5000 DH",
    subtotal: "5000 DH",
    tva: "1000 DH",
    total: "6000 DH",
    bankIce: "003408266000039",
    bank: "ATTIJARI WAFA BANK",
    account: "MARRAEKCHSTORY SARL",
    iban: "007450000488800000203358",
    phone: "+212 6 94 34 53 54",
    web: "www.marrakechstory.com"
  };
  ...
  function SocialMedia() {
    ...
    const invoice = h(
      ...
      h(
        "div",
        { className: "msa-print-page sm-invoice" },
        h(
          "div",
          { className: "sm-inv-head" },
          h("img", { src: "assets/logo.png", className: "sm-inv-logo", crossOrigin: "anonymous", onError: (e) => {
            e.target.style.display = "none";
          } }),
        ...
        h(
          ...
          h(
            "div",
            null,
            h("div", { className: "sm-inv-ch" }, "COORDONN\xC9ES :"),
        ...
        h("div", { className: "sm-inv-foot" }, "\u{1F4DE} " + SM_INVOICE.phone + "      " + SM_INVOICE.web + "      ICE : " + SM_INVOICE.bankIce),
        h("img", { src: "assets/stamp.png", className: "sm-inv-stamp", alt: "", crossOrigin: "anonymous", onError: (e) => {
          e.target.style.display = "none";
        } })
      )
    );
        ...
        h(
          ...
          h(
            ...
            h(
              "div",
              { className: "msa-field" },
              h("label", null, "Company (this month)"),
  ...
  function SearchResults({ q, data, route, openBooking, clear }) {
    ...
    return h(
      ...
      sec("Bookings", bk.map((b) => h("button", { key: b.id, className: "msa-line-item", onClick: () => openBooking(b) }, h("div", null, h("strong", null, b.client_name), h("span", { className: "msa-dim" }, " \xB7 " + (b.reference || "") + (isAdminRole() ? " \xB7 " + kr(b.selling_price) : ""))), h("span", { className: "msa-badge msa-st-" + b.status }, STATUS_LABEL[b.status])))),
      sec("Clients", cl.map((c) => h("button", { key: c.id, className: "msa-line-item", onClick: () => route("clients", c.name) }, h("div", null, h("strong", null, c.name), h("span", { className: "msa-dim" }, " \xB7 " + (c.phone || "") + " \xB7 " + kr(c.total_spent))), c.phone && h("span", { className: "msa-wa-btn" }, ICON.whatsapp())))),
      sec("Collaborators", su.map((s) => h("button", { key: s.id, className: "msa-line-item", onClick: () => route("suppliers") }, h("strong", null, s.name), h("span", { className: "msa-dim" }, " \xB7 " + (s.city || ""))))),
      sec("Tasks", tk.map((x) => h("button", { key: x.id, className: "msa-line-item", onClick: () => route("tasks") }, h("strong", null, x.title), h("span", { className: "msa-dim" }, " \xB7 " + (x.due || ""))))),
      sec("Requests", rq.map((l) => h("button", { key: l.id, className: "msa-line-item", onClick: () => route("requests") }, h("strong", null, l.name || l.email || "\u2014"), h("span", { className: "msa-dim" }, " \xB7 " + (reqTitle(l) || reqKindLabel(l))))))
    );
  }
  function uaParse(ua) {
    ua = ua || "";
  ...
  function Insights() {
    ...
    const load = useCallback(() => {
      ...
      sb.from("page_views").select("*").order("created_at", { ascending: false }).limit(8e3).then(({ data }) => {
        setRows(data || []);
    ...
    const saveGa = async () => {
      ...
      setGaSaving(false);
      if (!r || !r.ok) {
        setGaSaveErr(r && r.error || "Save failed");
        return;
      }
      setGa(null);
      loadGa();
    ...
    function metrics(list) {
      ...
      const bounces = list.filter((r) => {
        const sl = Array.isArray(r.sections) ? r.sections.length : 0;
        return (r.duration_seconds || 0) < 10 || sl <= 1;
      }).length;
    ...
    const active = new Set(rows.filter((r) => {
    ...
    const ccByName = {};
    cur.forEach((r) => {
      if (r.country && r.country_code) ccByName[r.country] = r.country_code;
    });
    ...
    const landings = tally(cur, (r) => {
      let l = (r.landing || "/").split("?")[0];
      return l || "/";
    }).slice(0, 6);
    ...
    const secEntries = function() {
      const m = {};
      cur.forEach((r) => (Array.isArray(r.sections) ? r.sections : []).forEach((s) => {
        m[s] = (m[s] || 0) + 1;
      }));
      return Object.entries(m).sort((a, b) => b[1] - a[1]);
    }();
    ...
    if (period === "24h") {
      ...
      cur.forEach((r) => {
        const hh = new Date(r.created_at).getHours();
        if (!isNaN(hh)) byH[hh]++;
      });
    ...
    cur.forEach((r) => {
      ...
      const set = sess[sid] || (sess[sid] = /* @__PURE__ */ new Set());
      (Array.isArray(r.sections) ? r.sections : []).forEach((s) => set.add(s));
    });
    ...
    if (period !== "all" && prev.length) {
      const dv = dPct(M.visits, Mp.visits);
      smart.push("Traffic is " + (dv >= 0 ? "up" : "down") + " " + Math.abs(dv) + "% versus the previous " + period + ".");
    }
    ...
    const kpi = (label, value, d, cls, sub) => h(
      "div",
      { className: "msa-kpi " + (cls || "msa-kpi-plain") },
      h("span", { className: "msa-kpi-label" }, label),
      h("span", { className: "msa-kpi-value" }, value),
    ...
    const srcBars = (entries, opt) => {
      opt = opt || {};
      if (!entries || !entries.length) return h("div", { className: "msa-empty" }, "No data yet.");
      const max = Math.max(1, ...entries.map((e) => e[1]));
      const tot = entries.reduce((s, e) => s + e[1], 0) || 1;
      return h("div", { className: "msa-srcbars" }, entries.map(([k, v], i) => h(
        "div",
        { key: i, className: "msa-srcbar" },
        h("span", { className: "msa-srcbar-l" }, opt.label ? opt.label(k) : k),
        h("div", { className: "msa-srcbar-track" }, h("div", { className: "msa-srcbar-fill", style: { width: v / max * 100 + "%", background: opt.color || void 0 } })),
        h("span", { className: "msa-srcbar-v" }, opt.pct ? Math.round(v / tot * 100) + "%" : nf(v))
      )));
    };
    const barChart = (data, color) => {
      const max = Math.max(1, ...data.map((d) => d.value));
      return h("div", { className: "msa-bars" }, data.map((d, i) => h("div", { key: i, className: "msa-bar-col" }, h("div", { className: "msa-bar-track" }, h("div", { className: "msa-bar-fill", style: { height: Math.round(d.value / max * 100) + "%", background: color || "var(--brand)" }, title: d.value + " visits" })), h("span", { className: "msa-bar-label" }, d.label))));
    };
    const card = (title, body, extra) => h("div", { className: "msa-card" }, h("div", { className: "msa-card-head" }, h("h3", null, title), extra || null), body);
    const periodSeg = h(
      "div",
      { className: "msa-seg msa-seg-sm" },
      [["24h", "Today"], ["7d", "7 days"], ["30d", "30 days"], ["all", "All time"]].map(([k, lbl]) => h("button", { key: k, className: period === k ? "active" : "", onClick: () => setPeriod(k) }, lbl))
    );
    ...
    const gaList = (title, items, live) => h(
      "div",
      { className: "msa-card msa-dash-box" },
      ...
      h("div", { className: "msa-dash-scroll" }, !items || !items.length ? h("div", { className: "msa-empty" }, "No data yet") : items.map((it, i) => h("div", { key: i, className: "msa-line-item" }, h("span", { title: it.key }, it.key || "(none)"), h("strong", { className: "msa-text-brand" }, nf(it.value)))))
    );
    ...
    const gaTrend = (data, label) => {
      ...
      return h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, label)),
        !d.length ? h("div", { className: "msa-empty" }, "No data yet") : h("div", { className: "msa-ga-trend" }, d.map((x, i) => h(
          "div",
          { key: i, className: "msa-ga-trend-col", title: nf(x.value) + " users \xB7 " + gaDate(x.key) },
    ...
    const gaSection = h(
      "div",
      { className: "msa-ga-section" },
      h(
        "h4",
        { className: "msa-section msa-section-row" },
        h("span", null, "Google Analytics"),
        ga && ga.configured && ga.ok ? h("span", { className: "msa-live" }, h("span", { className: "msa-live-dot" }), nf(ga.active || 0) + " active now") : null
      ),
      ga === null ? h("div", { className: "msa-card" }, h("div", { className: "msa-empty" }, "Connecting to Google Analytics\u2026")) : !ga.configured ? h("div", { className: "msa-card" }, h(
        "div",
        { className: "msa-ga-setup" },
      ...
      )) : !ga.ok || ga.error ? h("div", { className: "msa-card" }, h("div", { className: "msa-empty" }, "Google Analytics error: " + (ga.error || "unknown"))) : h(
        "div",
        null,
        h(
          "div",
          { className: "msa-kpi-grid" },
    ...
    return h(
      ...
      h(
        "div",
        { className: "msa-insights-bar" },
        periodSeg,
        h("span", { className: "msa-dim" }, M.visits + " visits \xB7 " + M.uniq + " unique in this period")
      ),
      rows.length === 0 ? h("div", { className: "msa-card" }, h("div", { className: "msa-empty" }, "No visits recorded yet. Data appears here automatically as people browse the public site.")) : h(
        "div",
        null,
        // PLAIN-LANGUAGE SUMMARY — the whole picture in a few sentences, first
        h(
          "div",
          { className: "msa-card msa-insights-sum" },
          h("div", { className: "msa-card-head" }, h("h3", null, "\u{1F4CC} At a glance")),
        ...
        // ONE TREND CHART
        card(
          period === "24h" ? "Visits by hour (today)" : "Visits \u2014 " + (period === "all" ? "all time" : "last " + period),
        ...
        card("From visit to contact", h("div", { className: "msa-funnel" }, funnel.map((f, i) => h(
          "div",
          { key: i, className: "msa-funnel-row" },
          h("div", { className: "msa-funnel-top" }, h("span", null, f.label), h("strong", null, f.n + " \xB7 " + Math.round(f.n / funnelTop * 100) + "%")),
  ...
  function Settings({ settings, onSaved }) {
    ...
    const saveAdminAccess = async () => {
      setAdminMsg("");
      if (!adminEmail.trim()) {
        setAdminMsg("Enter the admin login email.");
        return;
      }
      if (adminPass.length < 8) {
        setAdminMsg("Enter a password with at least 8 characters.");
        return;
      }
    ...
    const savePartner = async () => {
      setPMsg("");
      if (!pEmail.trim() || pPass.length < 8) {
        setPMsg("Enter the partner email and a password (min 8 chars).");
        return;
      }
      setPBusy(true);
    ...
    const revokePartner = async () => {
      ...
      setPBusy(true);
      const r = await callFn("manage-partner", { action: "revoke" });
      setPBusy(false);
      setPMsg(r && r.ok ? "Partner account removed." : "Failed.");
      if (r && r.ok) {
        setPEmail("");
        setPName("");
        onSaved && onSaved();
    ...
    return h(
      ...
      h(
        "header",
        { className: "msa-page-head msa-row" },
        h("div", null, h("h1", null, "Settings"), h("p", { className: "msa-subtitle" }, admin ? "Company info, invoices & admin controls" : "Your account")),
      ...
      admin ? h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Company")),
      ...
      admin ? h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Admin access")),
        ...
        h("button", { className: "msa-btn msa-btn-primary", style: { marginTop: 12 }, disabled: adminBusy, onClick: saveAdminAccess }, adminBusy ? "Saving\u2026" : "Update admin login")
      ) : null,
      admin ? h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Invoice & bank (admin only)")),
      ...
      admin ? h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Payment Information & Terms")),
      ...
      admin ? h(
        ...
        h(
          "div",
          { className: "msa-card-head" },
          h("h3", null, "Partner access (assistant account)"),
        ...
        h(
          ...
          settings && settings.partner_email && h("button", { className: "msa-btn", disabled: pBusy, onClick: revokePartner }, ICON.trash(), "Remove")
        )
      ) : null,
      !admin && h(
        "div",
        { className: "msa-card" },
        h("div", { className: "msa-card-head" }, h("h3", null, "Your password")),
        h(
          "div",
          { className: "msa-grid-2" },
  ...
  function Shell({ user, role, onLogout }) {
    ...
    useEffect(() => {
      touchPresence(tab);
      const t = setInterval(() => touchPresence(tab), 3e4);
      const onHide = () => touchPresence(tab);
    ...
    useEffect(() => {
      ...
      const sb = getSB();
      const timer = setInterval(reloadAll, 6e4);
    ...
    const body = () => {
      ...
      switch (tab) {
        ...
        case "requests":
          return h(Requests, { leads, bookings, reload: reloadAll, settings, suppliers });
        case "social":
          return h(SocialMedia, {});
        case "insights":
          return h(Insights, {});
        case "settings":
  ...
  function AdminRoot() {
    ...
    const applyUser = useCallback(async (u, resolvedRole) => {
      ...
      CURRENT_ROLE = r;
      CURRENT_EMAIL = (u.email || "").toLowerCase();
    ...
    if (user === void 0) return h("div", { className: "msa-login" }, h("div", { className: "msa-empty" }, "Loading\u2026"));
  ...
  let root = null;
  window.MS_AdminMount = function(el) {
    if (!el) return;
