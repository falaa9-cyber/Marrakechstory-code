(function() {
  ...
  function Dashboard({ bookings, tasks, leads, clients, go, openBooking, reload }) {
    ...
    return h(
      ...
      h(
        ...
        isAdmin ? h(
          "div",
          { className: "msa-dash-charts" },
          h(
            "div",
            { className: "msa-card" },
            h("div", { className: "msa-card-head" }, h("h3", null, "Cost breakdown"), h("button", { className: "msa-link", onClick: () => go("finance") }, "Finance \u2192")),
            h(
              "div",
          ...
          h(
            "div",
            { className: "msa-card" },
            h("div", { className: "msa-card-head" }, h("h3", null, "Revenue by month"), h("button", { className: "msa-link", onClick: () => go("finance") }, "Finance \u2192")),
  ...
  function BookingModal({ initial, onClose, onSaved, onView, suppliers }) {
    ...
    const addAct = (i) => setB((p) => {
      const a = [...p.daily_itinerary];
      a[i] = { ...a[i], activities: [...a[i].activities || [], { time: "09:00", type: "Transport", details: "" }] };
      return { ...p, daily_itinerary: a };
    });
    ...
    const save = async (closeAfter) => {
      ...
      setBusy(true);
      const row = { ...b, reference: b.reference || "MS-" + Math.random().toString(36).slice(2, 8).toUpperCase(), travelers: (+b.adults || 0) + (+b.kids || 0), updated_at: (/* @__PURE__ */ new Date()).toISOString() };
  ...
  function Shell({ user, role, onLogout }) {
    ...
    const body = () => {
      ...
      switch (tab) {
        ...
        case "finance":
          return isAdmin ? h(Finance, { bookings, suppliers }) : h(Dashboard, { bookings, tasks, leads, clients, go: setTab, openBooking, reload: reloadAll, isAdmin });
        case "tasks":
