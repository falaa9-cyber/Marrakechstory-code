(function() {
  ...
  function BookingModal({ initial, onClose, onSaved, onView, suppliers }) {
    ...
    const save = async (closeAfter) => {
      ...
      setBusy(true);
      const row = { ...b, reference: b.reference || "MS-" + Math.random().toString(36).slice(2, 8).toUpperCase(), travelers: (+b.adults || 0) + (+b.kids || 0), sell_currency: normalizeCurrency(b.sell_currency), cost_currency: normalizeCurrency(b.cost_currency, "MAD"), updated_at: (/* @__PURE__ */ new Date()).toISOString() };
