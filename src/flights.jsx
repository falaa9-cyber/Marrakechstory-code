...
function Flights() {
  ...
  const planWithUs = () => {
    ...
    const message = tx(
      ...
      `Hej Marrakech Story, jag vill ha hjälp med att planera flyg och en resa. Rutt: ${fromLabel} → ${toLabel}. Datum: ${formatMonthShort(outbound, lang)}${tripType === 'round_trip' ? ` till ${formatMonthShort(returnDate, lang)}` : ''} (${adults} vuxna, ${children} barn, ${infants} spädbarn). Klass: ${cabin}. Totalt: ${totalPax} resenär${totalPax > 1 ? 'er' : ''}.`
    );
        ...
        <aside className="flight-disclosure reveal">
          ...
          <p>
            {tx(
              ...
              'Marrakech Story visar flygpriser i realtid via affiliatepartners som Aviasales, Skyscanner och Kiwi.com. När du bokar via dessa länkar kan vi få en provision utan extra kostnad för dig. Vi är inte en flygbiljettagent och utfärdar inte biljetter eller hanterar flygbetalningar; alla bokningar slutförs på partnerns webbplats enligt deras villkor.'
            )}
          </p>
        </aside>
      </div>
